...
def check_database_status():
    ...
    # 1. 全ドキュメント数
    try:
        all_docs = db.client.table('documents').select('id, file_name, processing_status, workspace').execute()
    ...
    try:
        embedded_docs = db.client.table('documents').select('id, file_name').not_.is_('embedding', 'null').execute()
