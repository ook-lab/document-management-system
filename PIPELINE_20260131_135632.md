...
Generated: 2026-01-31T13:56:32.351772
Files: 32
...
- shared/pipeline/stage_g1_table_refiner.py (958 lines)
- shared/pipeline/stage_g2_text_refiner.py (917 lines)
...
```python
"""
Stage G1: Table Refiner（表専用整理）- Ver 6.4 物理分離対応

【設計 2026-01-31】Ver 6.4: e_content完全排除

役割: Fから届いた文字の最終研磨（改行・泣き別れ修正）のみ

============================================
【Ver 6.4】物理分離フロー:
  - F-10 で洗い替え完了済み → G1は研磨専念
  - G1 は「表(cell)」のみを受け取る（地文は G2 へ）
  - セル内の文字溶接（分断された単語の結合）に専念
  - Stage Eとの照合・置換は一切行わない
...
```python
"""
Stage G2: Text Refiner（テキスト専用整理）- Ver 6.4 物理分離対応

【設計 2026-01-31】Ver 6.4: e_content完全排除

役割: Fから届いたセグメントの論理的な接続（溶接）のみ

============================================
【Ver 6.4】物理分離フロー:
  - F-10 で洗い替え完了済み → G2は溶接専念
  - G2 は「地文(text)」のみを受け取る（表は G1 へ）
  - 表アンカー [→ TBL_xxx 参照] の完全保持を徹底
  - Stage Eとの照合・置換は一切行わない
...
class StageG2TextRefiner:
    ...
    def _process_v62(self, g2_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("[G2] 【Ver 6.4】地文専用レーン処理開始（溶接専念）")
    ...
    def _unify_text_v62(
        ...
        """
        【Ver 6.4】AI で断片的なテキストを一本の原稿に統合（溶接専念）
        ...
        prompt = f"""# 地文の溶接（Ver 6.4）

以下の断片的なテキストを、論理的に繋がる「一本の完璧な原稿」に溶接してください。

## 絶対ルール
1. **表アンカー [→ TBL_xxx 参照] は聖域** → 1文字も変えず必ず保持
2. OCRのゴミ（不自然な改行、謎の記号）を浄化
3. 文脈を考慮して自然な日本語に整形
4. 内容の追加・削除は禁止（溶接のみ）
