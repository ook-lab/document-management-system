-- 検索加点用の「日付だけ」の配列（date[]）。ix_date_signals からアプリが埋める。
-- unified_search_v2 v12: ix_search_dates を返す

ALTER TABLE public."09_unified_documents"
  ADD COLUMN IF NOT EXISTS ix_search_dates DATE[] NOT NULL DEFAULT ARRAY[]::DATE[];

COMMENT ON COLUMN public."09_unified_documents".ix_search_dates IS
  '検索加点用。全日を date[] にフラット化（レンジは日単位展開、月粒度はその月の全日）。';
...
'ハイブリッド検索（v12）: ix_search_dates（date[]）を返す。';
