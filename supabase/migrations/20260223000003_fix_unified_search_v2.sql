-- ============================================================
-- unified_search_v2 修正（v4）
-- Rawdata_FILE_AND_MAIL の実在カラムを確認した上で正しくマッピング
--
-- 非存在カラムと代替マッピング:
--   document_date  → display_sent_at::DATE（書類送付日。処理日の created_at は使わない）
--   summary        → display_subject → g21_articles[0].title の順でフォールバック
--   attachment_text→ display_post_text + g21_articles 本文 + g22 通知・イベントテキスト
--
-- 修正3: to_tsvector('japanese',...) 除去
--         → "japanese" text search configuration がインストールされていないため、
--            全文検索スコアを 0 固定にしてベクトル検索のみで動作させる
-- ============================================================
...
BEGIN
    ...
    WITH chunk_scores AS (
        SELECT
            ...
            -- ベクトル類似度（1 = 完全一致、0 = 無関係）
            (1.0 - (s.embedding <=> query_embedding))::FLOAT            AS raw_sim,
            -- 全文検索スコア（日本語テキスト設定が未インストールのため 0 固定、ベクトル検索に集中）
    ...
    SELECT
        ...
        -- attachment_text: display_post_text + G12表 + G21本文 + G22イベント・通知
        -- ※ g1x/g2x カラムは json.dumps() で二重シリアライズされているため
        --   #>> '{}' でアンラップしてから ::jsonb でパースする
...
'document_date=display_sent_at::DATE、'
'summary=display_subject/g21_articles[0].title、'
'attachment_text=display_post_text+g12 table descriptions+g21 bodies+g22 events/notices。';
