...
-- 1) 手動修正・除外管理テーブル
-- ============================================================
CREATE TABLE IF NOT EXISTS public."Kakeibo_Manual_Edits" (
    transaction_id text NOT NULL REFERENCES public."Rawdata_BANK_transactions"(id),
    manual_category_major text,
    manual_category_minor text,
    is_excluded boolean DEFAULT false, -- trueなら集計から除外（重複・相殺用）
    note text,
    updated_at timestamptz DEFAULT now(),
    PRIMARY KEY (transaction_id)
);

ALTER TABLE public."Kakeibo_Manual_Edits" ENABLE ROW LEVEL SECURITY;

-- ============================================================
-- 2) Viewの更新（CSV分類を無視 & 手動優先）
-- ============================================================
...
SELECT
    ...
    -- 除外フラグや振替フラグの解決
    COALESCE(m.is_excluded, false) AS is_excluded,  -- UIで除外されたものはtrue
