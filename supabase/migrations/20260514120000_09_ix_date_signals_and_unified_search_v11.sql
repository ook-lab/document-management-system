-- 検索ヒット・加点専用の日付（09 上の構造化＋統合本文から抽出した結果のみ）。meta とは別カラム。
-- unified_search_v2 に ix_date_signals を返す列を追加（v11）。

ALTER TABLE public."09_unified_documents"
  ADD COLUMN IF NOT EXISTS ix_date_signals JSONB;

COMMENT ON COLUMN public."09_unified_documents".ix_date_signals IS
  '検索加点用の日付シグナル（normalized_dates / normalized_ranges / partial_dates）。meta とは別。';

-- 旧 meta.date_signals を一度だけ移す（列が空の行のみ）
UPDATE public."09_unified_documents" u
SET ix_date_signals = u.meta -> 'date_signals'
WHERE u.meta ? 'date_signals'
  AND u.ix_date_signals IS NULL;

-- unified_search_v2 v11: ix_date_signals を返す
DO $$
DECLARE
    r RECORD;
BEGIN
...
'ハイブリッド検索（v11）: ix_date_signals 列を返す（検索加点用・meta 分離）。';
