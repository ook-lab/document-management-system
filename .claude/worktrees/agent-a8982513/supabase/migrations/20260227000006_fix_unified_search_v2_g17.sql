-- ============================================================
-- unified_search_v2 修正: g12_table_analyses → g17_table_analyses
-- ============================================================
-- 背景:
--   20260225000001_add_g17_table_analyses.sql で
--   g12_table_analyses カラムを g17_table_analyses にリネームしたが、
--   unified_search_v2 関数の本体が更新されておらず、
--   "column r.g12_table_analyses does not exist" エラーが発生していた。
-- ============================================================

-- file_url カラムが存在しない場合は追加（20260226000002 未適用環境対応）
...
'ハイブリッド検索（v7）: g12_table_analyses → g17_table_analyses 修正。file_url 対応。';

DO $$
BEGIN
    RAISE NOTICE '✅ 20260227000006_fix_unified_search_v2_g17.sql 適用完了';
    RAISE NOTICE '  - unified_search_v2: g12_table_analyses → g17_table_analyses に修正';
    RAISE NOTICE '  - file_url を RETURNS TABLE に追加（20260226000002 適用済み前提）';
END $$;
