-- ============================================================
-- source_id / source_type / display_type カラムを削除
-- ============================================================
-- 背景:
--   source_id   : source_url があれば不要（Drive URLからIDを抽出できる）
--   source_type : doc_type / workspace で代替可能
--   display_type: フロントエンドが使用していない
-- ============================================================

-- ============================================================
-- 1. dequeue_document を再定義（source_id / display_type を除去）
-- ============================================================
...
'リース方式デキュー v5: source_id / display_type を除去。source_url のみ使用。';

-- ============================================================
-- 2. unified_search_v2 を再定義（source_type / display_type を除去）
-- ============================================================
...
'ハイブリッド検索 v5: source_type / display_type を除去。source_url のみ使用。';
...
DO $$
BEGIN
    RAISE NOTICE '✅ drop_legacy_source_columns.sql 適用完了';
    RAISE NOTICE '  - dequeue_document: source_id / display_type を除去（v5）';
    RAISE NOTICE '  - unified_search_v2: source_type / display_type を除去（v5）';
    RAISE NOTICE '  - Rawdata_FILE_AND_MAIL: source_id / source_type / display_type カラム削除';
END $$;
