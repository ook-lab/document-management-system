...
Generated: 2026-01-31T07:31:25.129221
Files: 32
...
- shared/pipeline/stage_g1_table_refiner.py (775 lines)
- shared/pipeline/stage_g2_text_refiner.py (760 lines)
- shared/pipeline/stage_g_gate.py (327 lines)
- shared/pipeline/stage_g_refiner.py (701 lines)
- shared/pipeline/stage_h1_table.py (615 lines)
...
```python
"""
Stage G Gate: 純粋物流センター（Ver 5.8）

【設計 2026-01-30】物理的振分に専念
...
class StageGGate:
    ...
    def route(
        ...
        Returns:
            (g1_input, g2_input)
        """
        logger.info("[G-Gate] 【Ver 5.8】純粋物流センター起動")
    ...
    def _build_g1_package(
        ...
        return {
            'tables': tables,
            'e_content': e_content,  # G1がAI洗い替えに使う
            'f8_anchors': f8_anchors,  # G1が座標マッピングに使う
            'table_count': len(tables),
            'version': '5.8_pure_routing'
        }
    ...
    def _build_g2_package(
        ...
        return {
            'segments': segments,
            'e_content': e_content,  # G2がAI洗い替えに使う
...
class StageH1Table:
    ...
    def build_table_from_tagged_texts(
        ...
        AIのタグ（y_header）を信用しない。
        Suryaの物理座標（bbox）に基づき、数学的に最も近いヘッダーに強制配置。

        Args:
            tagged_texts: F-7からのタグ付きテキストリスト
            x_headers: X軸ヘッダーリスト
            y_headers: Y軸ヘッダーリスト
            f8_anchors: Suryaの物理座標リスト（Ver 5.8で必須）

        Returns:
            物理座標で正しく配置されたテーブル
        """
        logger.info(f"[Stage H1] 【Ver 5.8】物理座標マッピング開始: {len(tagged_texts)}テキスト")

        # f8_anchors から物理座標マップを構築
        ...
        if not valid_texts:
            logger.warning("[Stage H1] Ver5.8: テキストなし")
            return {
                'columns': [], 'rows': [], 'row_count': 0, 'col_count': 0,
                'source': 'ver5.8_coord', 'error': 'no_texts'
            }
        ...
        logger.info(f"[Stage H1] Ver5.8: X軸{len(x_headers)}個, Y軸{len(y_headers)}個, 座標付きY軸{len(y_header_coords)}個")

        # ============================================
        # 【核心】物理座標による強制マッピング
        # ============================================
        intersection_map = {}  # {(y_header, x_header): [texts]}
        coord_override_count = 0

        for text_item in valid_texts:
            text = text_item.get("text", "")
            ai_x_header = text_item.get("x_header", "")
            ai_y_header = text_item.get("y_header", "")

            # 物理座標による強制上書き
            final_y_header = ai_y_header

            if text in coord_map and y_header_coords:
                ...
                if nearest_y_header and nearest_y_header != ai_y_header:
                    logger.debug(f"[H1座標上書] '{text}': AI={ai_y_header} → 物理={nearest_y_header} (距離={min_distance:.1f})")
                    final_y_header = nearest_y_header
                    coord_override_count += 1

            # 交差点マップに追加
            key = (final_y_header, ai_x_header)
            if key not in intersection_map:
                intersection_map[key] = []
            intersection_map[key].append(text)

        logger.info(f"[Stage H1] 物理座標による上書き: {coord_override_count}件")
        ...
        logger.info(f"[Stage H1] 【Ver5.8】テーブル構築完了: {len(rows)}行 x {len(columns)}列, 座標上書き{coord_override_count}件")
        ...
        return {
            ...
            'coord_override_count': coord_override_count,
            'source': 'ver5.8_physical_coords'
        }
```
