-- =========================================
-- 検索関数を3-tier構造に完全移行
-- search_index (embedding) + source_documents (生データ) を JOIN
-- =========================================

BEGIN;

-- ■■■ 1. search_documents_final 関数（3-tier版）■■■
...
BEGIN
    ...
    SELECT DISTINCT ON (d.id)
        ...
        ((1 - (si.embedding <=> query_embedding)) * vector_weight +
         ts_rank(to_tsvector('simple', COALESCE(si.chunk_content, '')), plainto_tsquery('simple', query_text)) * fulltext_weight)::FLOAT AS combined_score,
        si.id AS small_chunk_id,
    ...
    INNER JOIN source_documents d ON si.document_id = d.id
    WHERE
        -- 年フィルタ
        (filter_year IS NULL OR EXTRACT(YEAR FROM d.document_date) = filter_year)
        -- 月フィルタ
...
BEGIN
    ...
    SELECT DISTINCT ON (d.id)
        ...
        d.metadata,
        d.document_date,
        (1 - (si.embedding <=> query_embedding))::FLOAT AS similarity_score,
        ts_rank(to_tsvector('japanese', COALESCE(si.chunk_content, '')), plainto_tsquery('japanese', query_text))::FLOAT AS text_rank,
        ((1 - (si.embedding <=> query_embedding)) * 0.7 +
         ts_rank(to_tsvector('japanese', COALESCE(si.chunk_content, '')), plainto_tsquery('japanese', query_text)) * 0.3)::FLOAT AS combined_score
    FROM search_index si
    INNER JOIN source_documents d ON si.document_id = d.id
    WHERE
...
-- 確認クエリ
-- =========================================
SELECT '✅ All SQL functions migrated to 3-tier architecture' as status;
