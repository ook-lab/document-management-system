-- ====================================================================
-- ハイブリッド検索関数のtext_weightを増やす
-- テキスト完全一致の重要性を向上（特に数値・単位の検索用）
-- ====================================================================

BEGIN;

-- 既存関数を削除
DROP FUNCTION IF EXISTS hybrid_search(vector, text, int, double precision, double precision, double precision, double precision);

-- ====================================================================
-- ハイブリッド検索関数（text_weight増加版）
-- ====================================================================
CREATE FUNCTION hybrid_search(
    query_embedding vector(1536),
    query_text text,
    match_count int DEFAULT 20,
    general_weight double precision DEFAULT 0.35,
    category_weight double precision DEFAULT 0.25,
    keywords_weight double precision DEFAULT 0.15,
    text_weight double precision DEFAULT 0.25
)
RETURNS TABLE (
    id uuid,
    product_name text,
...
BEGIN
    ...
    combined_scores AS (
        SELECT
            ...
            items.organization,
            items.current_price,
            items.current_price_tax_included,
            items.image_url,
            items.metadata,
    ...
    SELECT
        ...
        combined_scores.current_price,
        combined_scores.current_price_tax_included,
        combined_scores.image_url::text,
...
BEGIN
    RAISE NOTICE '====================================================================';
    RAISE NOTICE '✅ ハイブリッド検索関数を更新しました';
    RAISE NOTICE '   新しいウェイト: general=0.35, category=0.25, keywords=0.15, text=0.25';
    RAISE NOTICE '   （旧ウェイト: general=0.4, category=0.3, keywords=0.2, text=0.1）';
    RAISE NOTICE '====================================================================';
END $$;
