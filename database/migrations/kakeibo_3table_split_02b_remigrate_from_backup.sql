...
BEGIN
    ...
    RAISE NOTICE '現在のレシート数: % 件', current_receipt_count;
    RAISE NOTICE '現在のトランザクション数: % 件', current_trans_count;
    RAISE NOTICE '現在の標準化アイテム数: % 件', current_std_count;
    RAISE NOTICE '再移行を開始します...';
END $$;
...
-- ROW_NUMBER()を使って1対1のマッピングを保証

WITH old_numbered AS (
    SELECT
        t.*,
        r.id AS receipt_id,
        ROW_NUMBER() OVER (PARTITION BY r.id ORDER BY t.created_at, t.id) AS line_number
...
SELECT
    ...
    old.total_amount AS std_amount,              -- 最終支払金額
    NULL AS tax_amount,                          -- 旧テーブルにはtax_amountカラムなし
    'Migrated from OLD_BACKUP' AS calc_logic_log, -- マイグレーション由来であることを記録
...
-- ====================================================================
-- ステップ4: 99_lg_image_proc_logのreceipt_idを更新
-- ====================================================================
-- drive_file_idを使って、99_lg_image_proc_logのreceipt_idを正しいIDに更新
...
BEGIN
    ...
    FROM "99_lg_image_proc_log"
    WHERE receipt_id IS NOT NULL;

    RAISE NOTICE '✅ ステップ4完了: % 件のログにreceipt_idを設定', updated_count;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '次のステップ: Review UIでデータを確認してください';
END $$;
