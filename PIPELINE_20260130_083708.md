...
Generated: 2026-01-30T08:37:08.944738
Files: 32
...
- shared/pipeline/stage_f_visual.py (2737 lines)
...
class StageFVisualAnalyzer:
    ...
    def _f35_filter_and_columnize(self, blocks: List[Dict]) -> List[Dict]:
        ...
        # ============================================
        # Step 1: ノイズブロックの物理削除（強化版）
        # ============================================
        MIN_BLOCK_SIZE = 15  # 最小サイズ（量子化後）
        MIN_BLOCK_AREA = 300  # 最小面積（強化）
        ...
        for page_idx, page_blocks in pages.items():
            # ============================================
            # Step 2: 分割軸の決定（空白→罫線→密度）
            # ============================================
            split_axis, split_positions, split_reason = self._determine_split_axis(page_blocks)
    ...
    def _determine_split_axis(self, blocks: List[Dict]) -> tuple:
        ...
        # ============================================
        # 【Ver 3.4】垂直分割を全て試してから水平分割
        # 検出順: 垂直(空白→罫線→密度) → 水平(空白→罫線→密度)
        # ============================================

        # 各検出データを事前計算
        v_gaps = self._find_gaps(x_histogram, MIN_GAP, MAX_SPLITS)
        h_gaps = self._find_gaps(y_histogram, MIN_GAP, MAX_SPLITS)
        v_lines = self._find_lines(x_line_height, 300)
        h_lines = self._find_lines(y_line_width, 300)
        v_valleys = self._find_density_valleys(x_histogram, MAX_SPLITS)
        h_valleys = self._find_density_valleys(y_histogram, MAX_SPLITS)

        v_max_gap = max([g[1] for g in v_gaps]) if v_gaps else 0
        h_max_gap = max([g[1] for g in h_gaps]) if h_gaps else 0
        v_depth = max([v[1] for v in v_valleys]) if v_valleys else 0
        h_depth = max([v[1] for v in h_valleys]) if h_valleys else 0
        ...
        # 垂直1: 空白ガター
        if v_max_gap >= MIN_GAP:
            positions = sorted([g[0] for g in v_gaps[:MAX_SPLITS]])
            return ('vertical', positions, f'空白ガター検出(幅{v_max_gap})')

        # 垂直2: 罫線
        if v_lines:
            positions = sorted(v_lines[:MAX_SPLITS])
            return ('vertical', positions, f'罫線検出({len(v_lines)}本)')

        # 垂直3: 密度谷
        if v_valleys:
            positions = sorted([v[0] for v in v_valleys[:MAX_SPLITS]])
            return ('vertical', positions, f'密度谷検出(深さ{v_depth:.0f})')
        ...
        # 水平1: 空白ガター
        if h_max_gap >= MIN_GAP:
            positions = sorted([g[0] for g in h_gaps[:MAX_SPLITS]])
            return ('horizontal', positions, f'空白ガター検出(高さ{h_max_gap})')

        # 水平2: 罫線
        if h_lines:
            positions = sorted(h_lines[:MAX_SPLITS])
            return ('horizontal', positions, f'罫線検出({len(h_lines)}本)')

        # 水平3: 密度谷
        if h_valleys:
            positions = sorted([v[0] for v in h_valleys[:MAX_SPLITS]])
            return ('horizontal', positions, f'密度谷検出(深さ{h_depth:.0f})')
    ...
    def _find_density_valleys(self, histogram, max_count: int) -> List[tuple]:
        ...
        # 局所最小値を検出
        valleys = []
        for i in range(50, len(smoothed) - 50):
            if smoothed[i] < smoothed[i-20] and smoothed[i] < smoothed[i+20]:
                depth = min(smoothed[i-20], smoothed[i+20]) - smoothed[i]
                if depth > 0.5:  # 十分な深さ
                    valleys.append((i, depth))

        # 深さ順でソート
        valleys.sort(key=lambda x: x[1], reverse=True)
        return valleys[:max_count]
    ...
    def _build_f7_prompt(self, block_map: str) -> str:
        """F-7用プロンプト構築（予備知識付き・Stage E内容排除）"""
        # 予備知識の注入（Stage Eの内容は含めない）
        prior_knowledge = ""
        if hasattr(self, '_file_name') and self._file_name:
            prior_knowledge += f"- ファイル名: {self._file_name}\n"
        if hasattr(self, '_doc_type') and self._doc_type:
            prior_knowledge += f"- 推定ドキュメントタイプ: {self._doc_type}\n"
        if hasattr(self, '_workspace') and self._workspace:
            prior_knowledge += f"- ワークスペース: {self._workspace}\n"

        prior_section = f"""## 【予備知識】
{prior_knowledge}
**注意**: 上記は参考情報のみ。実際の内容は画像から読み取ってください。

""" if prior_knowledge else ""

        return f"""# F-7: Text Extraction / テキストの鬼

{prior_section}## Mission
...
"""
    ...
    # ============================================
    # Ver 3.1: 水平優先・非破壊的スマートクロッピング
    # ============================================

    def _detect_gutters(self, image: Image.Image, direction: str = 'horizontal', min_gap: int = 30, max_gutters: int = 4) -> List[int]:
        """
        画像内のガター（空白の道）を検出 - 最大隙間優先版

        Args:
            image: PIL Image
            direction: 'horizontal'（水平ガター）or 'vertical'（垂直ガター）
            min_gap: 最小ガター幅（ピクセル）
            max_gutters: 最大ガター数（内部仕切り線の上限）

        Returns:
            ガター位置のリスト（ピクセル座標）- サイズ順で上位max_gutters個
        """
        import numpy as np
        ...
        if direction == 'horizontal':
            ...
            y = min_gap
            while y < height - min_gap:
                row_mean = np.mean(pixels[y, :])
                if row_mean > 250:
                    start = y
                    while y < height and np.mean(pixels[y, :]) > 250:
                        y += 1
                    gap_size = y - start
                    if gap_size >= min_gap:
                        center = start + gap_size // 2
                        gutter_candidates.append((center, gap_size))
                else:
                    y += 1
        else:
            width = pixels.shape[1]
            x = min_gap
            while x < width - min_gap:
                col_mean = np.mean(pixels[:, x])
                if col_mean > 250:
                    start = x
                    while x < width and np.mean(pixels[:, x]) > 250:
                        x += 1
                    gap_size = x - start
                    if gap_size >= min_gap:
                        center = start + gap_size // 2
        ...
        # サイズ順でソートして上位max_gutters個を採用
        if len(gutter_candidates) > max_gutters:
            logger.info(f"[Safety] {direction} gutters limited: {len(gutter_candidates)} → top {max_gutters} by size")
            gutter_candidates.sort(key=lambda x: x[1], reverse=True)
            gutter_candidates = gutter_candidates[:max_gutters]

        # 位置順にソートして返す
        gutter_candidates.sort(key=lambda x: x[0])
        return [g[0] for g in gutter_candidates]
    ...
    def _smart_crop_patches(
        ...
        """
        Ver 3.3: 横分割時もインデックス列を複製

        【核心】横に切っても、左端のインデックス列（偏差値等）を全パッチに随伴
        これにより「73偏差値」「70偏差値」の紐付けが全セクションで維持される
        """
        img_width, img_height = image.size
        patches = []

        # Step 1: 水平ガターを検出
        h_gutters = self._detect_gutters(image, direction='horizontal', min_gap=30)
        logger.info(f"[SmartCrop] 水平ガター検出: {len(h_gutters)}個 at {h_gutters}")

        # Step 2: 水平セクションに分割
        h_boundaries = [0] + h_gutters + [img_height]
        h_sections = []
        for i in range(len(h_boundaries) - 1):
            y_start = max(0, h_boundaries[i] - overlap if i > 0 else 0)
            y_end = min(img_height, h_boundaries[i + 1] + overlap if i < len(h_boundaries) - 2 else img_height)
            if y_end - y_start > 50:
                h_sections.append((y_start, y_end))

        logger.info(f"[SmartCrop] 水平セクション: {len(h_sections)}個")

        if len(h_sections) <= 1:
            h_sections = [(0, img_height)]

        # ============================================
        # 【重要】インデックス列（左端）を検出・抽出
        # ============================================
        # 左端の10%をインデックス列とみなす（偏差値表対策）
        INDEX_COLUMN_WIDTH_RATIO = 0.12  # 左端12%
        index_col_width = int(img_width * INDEX_COLUMN_WIDTH_RATIO)

        # Step 3: 各水平セクションを処理（インデックス列を全セクションに複製）
        total_sections = len(h_sections)

        for sec_idx, (y_start, y_end) in enumerate(h_sections):
            section_height = y_end - y_start

            # このセクションのインデックス列を抽出
            index_col_img = image.crop((0, y_start, index_col_width + overlap, y_end))

            # データ部分（インデックス列の右側）を抽出
            data_section_img = image.crop((0, y_start, img_width, y_end))

            # 垂直分割が必要か判定
            num_columns = len(column_boundaries) - 1 if column_boundaries and len(column_boundaries) > 1 else 1
            v_gutters = self._detect_gutters(data_section_img, direction='vertical', min_gap=30) if num_columns > 1 else []

            if num_columns <= 1 or len(v_gutters) < num_columns - 1:
                # ============================================
                # 横分割のみ: インデックス列を左に結合してパッチ化
                # ============================================
                if sec_idx == 0:
                    # 最初のセクション（ヘッダー含む）はそのまま
                    patches.append({
                        'image': data_section_img,
                        'type': 'h_section_header',
                        'info': f'セクション{sec_idx + 1}/{total_sections}（ヘッダー）',
                        'patch_index': sec_idx,
                        'total_patches': total_sections,
                        'is_continuation': False
                    })
                else:
                    # 2セクション目以降: 最初のセクションのインデックス列を複製
                    first_section_index = image.crop((0, h_sections[0][0], index_col_width + overlap, h_sections[0][1]))

                    # インデックス列のサイズを現在のセクションに合わせてリサイズ
                    if first_section_index.height != section_height:
                        # 高さを合わせる（偏差値の目盛りをスケール）
                        first_section_index = first_section_index.resize(
                            (first_section_index.width, section_height),
                            Image.Resampling.LANCZOS
                        )

                    # インデックス列 + データ部分を結合
                    combined_width = first_section_index.width + data_section_img.width
                    combined_img = Image.new('RGB', (combined_width, section_height), (255, 255, 255))
                    combined_img.paste(first_section_index, (0, 0))
                    combined_img.paste(data_section_img, (first_section_index.width, 0))

                    patches.append({
                        'image': combined_img,
                        'type': 'h_section_with_index',
                        'info': f'セクション{sec_idx + 1}/{total_sections}（インデックス列付き継続）',
                        'patch_index': sec_idx,
                        'total_patches': total_sections,
                ...
                for col_idx in range(num_columns):
                    col_start = int(column_boundaries[col_idx] * img_width / QUANTIZE_GRID_SIZE)
                    col_end = int(column_boundaries[col_idx + 1] * img_width / QUANTIZE_GRID_SIZE) if col_idx + 1 < len(column_boundaries) else img_width

                    px_start = max(0, col_start - overlap)
                    px_end = min(img_width, col_end + overlap)

                    col_img = data_section_img.crop((px_start, 0, px_end, section_height))
                    ...
                    if col_idx == 0:
                        ...
                        patches.append({
                            'image': col_img,
                            'type': 'v_column_index',
                            'info': f'セクション{sec_idx + 1} 列1（インデックス列）',
                            'patch_index': len(patches),
                            'total_patches': total_sections * num_columns,
                            'is_continuation': sec_idx > 0
                        })
                    else:
                        # データ列: インデックス列を左に結合
                        combined_width = index_col_img.width + col_img.width
                        combined_img = Image.new('RGB', (combined_width, section_height), (255, 255, 255))
                        combined_img.paste(index_col_img, (0, 0))
                        combined_img.paste(col_img, (index_col_img.width, 0))

                        patches.append({
                            'image': combined_img,
                            'type': 'v_column_with_index',
                            'info': f'セクション{sec_idx + 1} 列{col_idx + 1}/{num_columns}（インデックス列付き）',
                            'patch_index': len(patches),
                            'total_patches': total_sections * num_columns,
                            'is_continuation': sec_idx > 0
                        })
        ...
        for i, patch in enumerate(patches):
            patch['patch_index'] = i
            patch['total_patches'] = len(patches)

        logger.info(f"[SmartCrop] 最終パッチ数: {len(patches)}")
        return patches
    ...
    def _f7_path_a_chunk_extraction(
        ...
        """
        F-7 Path A: Ver 3.1 水平優先・非破壊的スマートクロッピング

        【設計原則】
        1. 水平ガター優先: まず上下に分割
        2. 垂直は慎重に: 明確なガターがある場合のみ
        3. インデックス列複製: 偏差値表対策
        4. のりしろ付き: 文字欠け防止
        5. 賢い諦め: 不明確なら丸ごと渡す
        """
        import tempfile
        f7_start = time.time()
        logger.info(f"[F-7] Path A - チャンク{chunk_idx + 1} SmartCrop開始")
        ...
        # スマートクロッピングでパッチを生成
        patches = self._smart_crop_patches(original_image, column_boundaries, overlap=50)
        ...
        for patch_idx, patch in enumerate(patches):
            ...
            try:
                ...
                prompt = self._build_f7_smart_prompt(
                    block_map, chunk_idx, chunk_start_page, patch_info, patch_type, len(patches),
                ...
                all_full_texts.append(result.get("full_text_ordered", ""))
...
## ⚠️ 重要：継続パッチの処理
- **これは分割されたパッチの {patch_index + 1}枚目/{total_patches}枚 です**
- **前のパッチからの継続です。1つの大きなテーブルの一部として処理してください**
- 左端にインデックス列（偏差値等）が複製されています
- このインデックス列の数値を基準に、右側のデータを正確に紐付けてください
- **表のtable_idは全パッチで同じIDを使用し、後段でマージ可能にしてください**
...
- **全体を俯瞰して、左右の対比（例：5Aと5B）を重視して抽出してください**
"""
        elif patch_type in ('v_column_with_index', 'h_section_with_index'):
            context = f"""
## 画像情報（インデックス列付き分割）
- {patch_info}
- ページ番号: {start_page + 1}ページ目
- パッチ: {patch_index + 1}/{total_patches}
- **左端にインデックス列（偏差値・時限等）が複製されています**
- インデックス列の各数値と、右側のデータ行を正確に対応させてください
- **行ごとに「インデックス値 → データ」の対応を維持すること**
"""
        elif patch_type == 'h_section_header':
            context = f"""
## 画像情報（ヘッダーセクション）
- {patch_info}
- ページ番号: {start_page + 1}ページ目
- パッチ: {patch_index + 1}/{total_patches}
- **これは表のヘッダー部分を含む最初のセクションです**
- 表のタイトル、列名、データの最初の部分を正確に抽出してください
"""
        else:
            context = f"""
## 画像情報
