...
生成日時: 2026-02-01T07:53:49.256880
---
## サマリー
- プロンプト定義数: 8
- LLM呼び出し箇所: 12
...
- 種別: function
- 説明: 【Ver 6.5】AIは文字を座標付きで読むだけ。ID紐付けは一切禁止。

        AIの仕事: 画像内の全文字を座標付きで出力
        プログラムの仕事(F7.5): Surya座標との最短距離マッチング

**プロンプト全文:**
```
画像内の全ての文字を、位置座標付きで出力してください。

出力形式:
{"texts": [{"text": "文字列", "x": 123, "y": 456}, ...]}

ルール:
- x, y は文字の左上座標（ピクセル単位、画像内の実座標）
- 赤/黄色のIDラベルは無視（読み取り対象外）
- 文字のみを出力（IDとの紐付けは後続処理で行う）
- 全ての文字を漏れなく拾う
```

### 3. _build_f8_prompt (stage_f_visual.py)
- 種別: function
- 説明: 【Ver 4.0】F-8用プロンプト構築（座標排除版）
