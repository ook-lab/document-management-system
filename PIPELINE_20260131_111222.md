...
Generated: 2026-01-31T11:12:22.469680
Files: 32
...
- shared/pipeline/stage_f_visual.py (3095 lines)
...
class StageFVisualAnalyzer:
    ...
    def _build_f7_prompt(self) -> str:
        """
        【Ver 6.2】物理座標ベース・テキスト溶接プロンプト（自律検出版）

        AIに「ID:テキスト」形式で出力させることで：
        - 物理的根拠をテキストとして永続化
        - H1での座標再計算を廃止
        - デバッグの透明性を確保
        """
        return """# 物理座標ベース・テキスト溶接指示（Ver 6.2）

## あなたの任務
画像内の「黄色ラベル（#ID）」が付与された各領域の文字を読み取り、それらがどの「ヘッダーID」に物理的に属するかを特定して構造化せよ。
「見た目」や「推論」は禁止する。すべて **#ID の物理的な相対位置** のみを根拠とせよ。

## 1. ヘッダーの自律検出
まず画像を観察し、以下を特定せよ：
- **X軸ヘッダー**: 画像の**最上部**にある列見出し（日付: 2/1, 2/2 など、またはカテゴリ名）の #ID を記録
- **Y軸ヘッダー**: 画像の**最左端**にある行見出し（数値: 74, 73 など、または項目名）の #ID を記録

検出した各ヘッダーは、そのヘッダー自身も rows に出力せよ（type="x_header" または "y_header"）。

## 2. 物理マッピングの鉄則
各データ（学校名等）の #ID について：
1. 画像上の **物理的な位置関係** から、最も近い X軸ヘッダーの #ID を特定
2. 同様に、最も近い Y軸ヘッダーの #ID を特定
3. **「ID:テキスト」形式** で記録（例: "5:2/1", "10:74"）

## 3. 例外処理
**物理的にどのヘッダーとも結びつかない #ID のみ**、例外的に内容から役割を推論せよ。その場合、ヘッダー情報は `"none"` とせよ。

## 4. 出力形式（CSV型JSON：ID＋ラベル溶接）
...
```json
{
  ...
  "rows": [
    ...
    ["2", "2/2", "none", "none", "x_header"],
    ["10", "74", "none", "none", "y_header"],
    ["11", "73", "none", "none", "y_header"],
    ["20", "筑波大駒場", "1:2/1", "10:74", "cell"],
    ["21", "開成", "2:2/2", "10:74", "cell"],
    ["22", "聖光学院", "1:2/1", "11:73", "cell"],
    ["99", "2026年度成績表", "none", "none", "title"]
  ]
}
```

### 列の意味（cols）
- `id`: このテキスト自身の #ID（黄色ラベルの数字）
- `text`: 読み取ったテキスト内容
- `xh`: 紐付くX軸ヘッダー（**"ID:テキスト"** 形式、例: "1:2/1"）。ヘッダー自身やタイトルは "none"
- `yh`: 紐付くY軸ヘッダー（**"ID:テキスト"** 形式、例: "10:74"）。ヘッダー自身やタイトルは "none"
- `type`: "cell", "x_header", "y_header", "title", "note"

## 重要なルール
1. **全テキスト抽出**: 黄色ラベル付きの全領域を漏れなく読み取れ
2. **省略禁止**: 似た内容でも全て個別に出力
3. **ID:テキスト形式厳守**: xh, yh は必ず "ID:テキスト" 形式で記述（例: "1:2/1"）
4. **物理距離優先**: 複数ヘッダーに近い場合、物理的に最も近いIDを採用
5. **ヘッダーも出力**: 検出した全ヘッダーを type="x_header"/"y_header" として出力

## 禁止事項
- 「見た目」や「意味」による推論（物理座標のみを信じよ）
- データの要約や省略
- 空セルの推測（見えないものは出力しない）
- 冗長なJSONキー繰り返し（値のみの配列形式を厳守）
"""
