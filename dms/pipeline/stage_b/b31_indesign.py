"""
B-31: InDesign Processor（Adobe InDesign 由来 PDF 専用）

Adobe InDesign で作成された PDF を処理する。
InDesign PDF の特徴:
- テキストが正規の PDF テキストオブジェクトとして埋め込まれている
- 複数ページ・複雑なレイアウト（マスターページ・段組み）
- pdfplumber での文字抽出精度が高い
- テキストスレッド（フレーム間テキスト連結）が存在することがある

処理方針:
- pdfplumber で文字を抽出し、テキストボックス単位でログ出力
- 段組みを考慮した Y/X クラスタリングでブロックを構築
"""
...
class B31InDesignProcessor:
    ...
    def process(self, file_path: Path, masked_pages=None, log_file=None) -> Dict[str, Any]:
        ...
        if log_file:
            _sink_id = logger.add(
                ...
                filter=lambda r: "[B-31]" in r["message"],
                level="DEBUG",
                encoding="utf-8",
            )
    ...
    def _process_impl(self, file_path: Path, masked_pages=None) -> Dict[str, Any]:
        logger.info(f"[B-31] InDesign PDF処理開始: {file_path.name}")

        try:
            import pdfplumber
        except ImportError:
            logger.error("[B-31] pdfplumber がインストールされていません")
            return self._error_result("pdfplumber not installed")

        try:
            with pdfplumber.open(str(file_path)) as pdf:
                logger.info(f"[B-31] PDF情報:")
                logger.info(f"[B-31]   ├─ ページ数: {len(pdf.pages)}")
                logger.info(f"[B-31]   ├─ メタデータ: {pdf.metadata}")
                ...
                for page_num, page in enumerate(pdf.pages):
                    if _masked and page_num in _masked:
                        logger.debug(f"[B-31] ページ{page_num+1}: マスク → スキップ")
                        continue

                    logger.info(f"[B-31] ========== ページ {page_num+1}/{len(pdf.pages)} ==========")
                    logger.info(
                        f"[B-31]   ├─ ページサイズ: {page.width:.1f} x {page.height:.1f} pt"
                    )
                    logger.info(
                        f"[B-31]   ├─ chars={len(page.chars or [])} "
                    ...
                    logger.info(
                        f"[B-31]   └─ テキストボックス={len(merged_boxes)} "
                ...
                tags = {
                    ...
                    'processor': 'b31_indesign',
                }

                logger.info(f"[B-31] ========== 抽出テキスト全文（位置情報付き） ==========")
                for idx, block in enumerate(logical_blocks):
                    bbox = block.get('bbox', (0, 0, 0, 0))
                    logger.info(
                        f"[B-31] Block #{idx+1} | Page:{block.get('page', 0)} | "
                        f"Bbox:({bbox[0]:.1f},{bbox[1]:.1f},{bbox[2]:.1f},{bbox[3]:.1f})"
                    )
                    logger.info(f"[B-31] Text: {block.get('text', '')!r}")

                purged_pdf_path = self._purge_extracted_text(file_path, all_words, all_tables)
                logger.info(f"[B-31] purged PDF 生成完了: {purged_pdf_path.name}")
            ...
            logger.error(f"[B-31] 処理エラー: {e}", exc_info=True)
    ...
    def _merge_nearby_boxes(self, textboxes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"[B-31] マージ: {len(textboxes)}個 → {len(merged)}個")
        return merged
    ...
    def _extract_tables(self, page, page_num: int) -> List[Dict[str, Any]]:
        ...
        for idx, table in enumerate(page.find_tables()):
            ...
            logger.info(f"[B-31] Table {idx} (Page {page_num}): {rows}行×{cols}列")
            if data:
                for row_idx, row in enumerate(data):
                    logger.info(f"[B-31] Table {idx} 行{row_idx}: {row}")
            tables.append({
                'page': page_num,
                'index': idx,
                'bbox': table.bbox,
    ...
    def _purge_extracted_text(
        ...
        try:
            ...
            logger.error("[B-31] PyMuPDF がインストールされていません")
            return file_path
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                if page_words:
                    logger.info(f"[B-31] ページ {page_num+1}: {len(page_words)}単語を削除")
                    for word in page_words:
                ...
                if page_tables:
                    logger.info(f"[B-31] ページ {page_num+1}: {len(page_tables)}表の罫線を削除")
            ...
            purged_pdf_path = purged_dir / f"b31_{file_path.stem}_purged.pdf"
            doc.save(str(purged_pdf_path))
            doc.close()

            logger.info(f"[B-31] 削除した単語: {deleted_words}個")
            logger.info(f"[B-31] 削除した表罫線: {deleted_graphics}個")
            logger.info(f"[B-31] purged PDF 保存先: {purged_pdf_path}")
            return purged_pdf_path

        except Exception as e:
            logger.error(f"[B-31] テキスト削除エラー: {e}", exc_info=True)
            return file_path
