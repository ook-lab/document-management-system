...
from dms.pipeline.stage_g.merged_cell_grid import is_merge_placeholder, resolve_horizontal_merges_in_row
...
def _labels_from_col_analysis(meta: Dict[str, Any], ncols: int) -> List[str]:
    """G32 col_analysis の category 名を列ラベル候補にする。"""
    ...
    for c in range(max_c):
        ...
        if c == 0:
            for ri in range(h_end):
                ...
                if t and t not in _GENERIC_HEADER_LABELS:
                    parts.append(t)
        else:
            row_order = list(hr) if hr else [0]
            if last_hr not in row_order:
                row_order.append(last_hr)
            for ri in row_order:
                if ri >= len(rows):
                    continue
                row = rows[ri]
                ...
                if t and not parts:
                    parts.append(t)
        if not parts and c < len(ca_labels):
            ca = ca_labels[c]
            if ca and ca not in _GENERIC_HEADER_LABELS:
                parts.append(ca)
        labels.append(parts[0] if len(parts) == 1 else (" / ".join(parts) if parts else f"列{c + 1}"))
    return labels


def resolve_ui_column_labels(ui_table: Dict[str, Any]) -> List[str]:
    """UI / MD 用列見出し（G28 正本・col_map・G32 を順に参照）。"""
    rows = ui_table.get("rows") or []
    if not rows:
        return []
    meta = ui_table.get("metadata") or {}
    ...
    if meta.get("f56_split_axis") == "col" or meta.get("split_from"):
        ...
        if isinstance(display, list) and len(display) >= max_c:
            labels = [_cell_text_flat(x) for x in display[:max_c]]
            if not all(t in ("", "授業名", "科目") or not t for t in labels):
                return [t or f"列{i + 1}" for i, t in enumerate(labels)]
        return _labels_for_section_header(rows, meta, max_c)

    display = meta.get("display_column_labels")
    if isinstance(display, list) and len(display) >= max_c:
        labels = [_cell_text_flat(x) for x in display[:max_c]]
        if not all(t in ("", "授業名") for t in labels):
            return [t or f"列{i + 1}" for i, t in enumerate(labels)]
...
def infer_column_labels(ui_table: Dict[str, Any]) -> List[str]:
    ...
    （G62 が付けたデータ開始行。header_rows が過剰でもデータ行を列見出しに混ぜない）。
