"""後方互換: F50 意味付与は廃止。G26（G11 経由）を使うこと。"""
...
from dms.pipeline.stage_g.g26_line_semantics import G26SemanticAIError, G26_LINE_SEMANTICS_CONTRACT
from dms.pipeline.stage_g.g26_semantic_estimator import G26SemanticEstimator
from dms.pipeline.stage_f.f50_d_line_split import apply_d_line_split_structured_tables
...
def run_f50_semantics(
    ...
    *,
    ...
    if not digest.get("available"):
        logger.info("[F50] stage_d_line_digest なし → スキップ（G26 へ委譲なし）")
...
def run_f50_apply_split(
    chain_context: Optional[Dict[str, Any]],
    structured_tables: List[Dict[str, Any]],
) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ctx = dict(chain_context or {})
    digest = dict(ctx.get("stage_d_line_digest") or {})
    tables, meta = apply_d_line_split_structured_tables(structured_tables, digest)
    if meta.get("contract"):
        ctx["d_line_split_contract"] = meta["contract"]
    return ctx, tables


def run_f50_semantics_and_split(
    chain_context: Optional[Dict[str, Any]],
    structured_tables: List[Dict[str, Any]],
    *,
    document_id: Optional[str] = None,
) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ctx, tables = run_f50_semantics(
        chain_context, structured_tables, document_id=document_id
    )
    return run_f50_apply_split(ctx, tables)
