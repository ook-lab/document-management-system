...
Generated: 2026-01-30T17:15:47.071334
Files: 32
...
- shared/pipeline/stage_f_visual.py (2918 lines)
- shared/pipeline/stage_g1_table_refiner.py (776 lines)
- shared/pipeline/stage_g2_text_refiner.py (760 lines)
- shared/pipeline/stage_g_gate.py (526 lines)
- shared/pipeline/stage_g_refiner.py (701 lines)
- shared/pipeline/stage_h1_table.py (564 lines)
...
F7_MODEL_IMAGE = "gemini-2.5-flash-lite"  # 【Ver 5.6】画像用（自律ヘッダー特定）
...
class StageFVisualAnalyzer:
    ...
    def _build_f7_prompt(self) -> str:
        ...
        - 各テキストにヘッダータグを付与
        """
        return """# テキスト抽出指示（Ver 5.6 自律モード）

## あなたのタスク
この画像から**全てのテキストを抽出**し、表構造を自律的に解析してください。

## 自律解析の手順

### STEP 1: ヘッダーの特定
画像を観察し、以下を特定してください：
- **X軸ヘッダー**: 画像の**最上部**にある列見出し（日付、カテゴリ名、時間帯など）
- **Y軸ヘッダー**: 画像の**最左部**にある行見出し（数値、偏差値、項目名など）

ヒント: ヘッダーは通常、他のセルより目立つ位置・スタイルにあります。

### STEP 2: 各テキストのタグ付け
抽出した各テキストについて：
1. そのテキストの**真上**にあるX軸ヘッダーを特定
2. そのテキストの**真左**にあるY軸ヘッダーを特定
3. どちらのヘッダー下にもない場合は untagged_texts へ

## 出力形式（厳守）

```json
{
  "x_headers": ["列ヘッダー1", "列ヘッダー2", "..."],
  "y_headers": ["行ヘッダー1", "行ヘッダー2", "..."],
  "tagged_texts": [
    {
      "text": "セル内容",
      "x_header": "対応する列ヘッダー",
...
## 重要なルール
1. **全テキスト抽出**: 画像内の全ての文字を漏れなく抽出
2. **省略禁止**: 似た内容でも全て個別に出力
3. **ヘッダー判定は位置ベース**: テキストの座標位置から最も近いヘッダーを選択
4. **曖昧な場合は最寄り**: 複数ヘッダーに近い場合、最も近いものを採用
    ...
    def _f7_path_a_chunk_extraction(
        ...
        for patch_idx, patch in enumerate(patches):
            ...
            try:
                ...
                logger.info(f"[F-7] パッチ{patch_idx + 1} Ver5.6収集: {len([t for t in all_extracted_texts if t.get('patch_idx') == patch_idx])}テキスト, x_headers={len(x_headers)}, y_headers={len(y_headers)}")
...
```python
"""
Stage G Gate: 物流センター（Ver 5.7）

【設計 2026-01-30】三位一体（F7+F8+E）結合

役割: F7/F8/E を束ねて H に運ぶだけ
      勝手な分割・廃棄は物理的に禁止

============================================
入力:
  - stage_e_result: デジタル文字（正確）
  - stage_f_payload: F7論理データ + F8物理座標

出力:
  - g1_input: 統合パケット（表用）
  - g2_input: 統合パケット（テキスト用）

三大機能:
  1. 抱き合わせ (Binding): F7 + F8 + E を一つのパケットに
  2. 洗い替え (Scrubbing): F7のOCRをEの正確文字で置換
  3. 直送 (Relay): 分割・廃棄せずそのまま転送
============================================
"""
...
class StageGGate:
    ...
    def route(
        ...
        Returns:
            (g1_input, g2_input)
        """
        logger.info("[G-Gate] 【Ver 5.7】物流センター起動")
        logger.info("  ├─ 禁止: 分割・廃棄・勝手な判断")
        logger.info("  └─ 許可: 抱き合わせ・洗い替え・直送")
        ...
        e_tokens = self._tokenize_e_content(e_content)
        logger.info(f"[G-Gate] E素材: {len(e_content)}文字 → {len(e_tokens)}トークン")
        ...
        # ============================================
        # Step 2: 抱き合わせ (Binding)
        # F7 + F8 + E を統合パケットに
        # ============================================
        table_packets = self._bind_table_packets(f7_tables, f8_anchors, e_tokens)
        text_packets = self._bind_text_packets(f_text_blocks, f8_anchors, e_tokens)

        logger.info(f"[G-Gate] 抱き合わせ完了: 表{len(table_packets)}パケット, テキスト{len(text_packets)}パケット")

        # ============================================
        # Step 3: 洗い替え (Scrubbing)
        # F7のOCR文字をEの正確文字で置換
        # ============================================
        table_packets = self._scrub_packets(table_packets, e_tokens)
        text_packets = self._scrub_packets(text_packets, e_tokens)

        logger.info("[G-Gate] 洗い替え完了")

        # ============================================
        # Step 4: 直送 (Relay)
        # 分割・廃棄せずそのまま転送
        # ============================================
        g1_input = self._build_g1_relay(table_packets, e_content)
        g2_input = self._build_g2_relay(text_packets, post_body, e_content)

        logger.info(f"[G-Gate] 直送準備完了:")
        logger.info(f"  ├─ G1: {len(g1_input.get('tables', []))}表")
        logger.info(f"  └─ G2: {len(g2_input.get('segments', []))}セグメント")
    ...
    def _tokenize_e_content(self, e_content: str) -> set:
        """Stage Eのコンテンツをトークン化"""
        if not e_content:
            return set()
    ...
    def _extract_f8_anchors(self, stage_f_payload: Dict[str, Any]) -> List[Dict]:
        """F8の物理座標（Surya BBox）を抽出"""
        anchors = []

        # text_source.blocks から座標を取得
    ...
    def _bind_table_packets(
        ...
        for tbl in f7_tables:
            ...
            # F8座標を探す（同一ページのアンカーを紐付け）
            page = tbl.get('page', 0)
            related_anchors = [
                a for a in f8_anchors
                if a.get('page', -1) == page
            ]
            ...
            packet = {
                ...
                # E参照（洗い替え用）
                'e_token_count': len(e_tokens),
                # メタデータ
                'source': tbl.get('source', 'f7'),
                'block_id': tbl.get('block_id', ''),
            }
    ...
    def _scrub_packets(self, packets: List[Dict], e_tokens: set) -> List[Dict]:
        """
        パケット内のOCRテキストをStage Eの正確文字で置換
        """
        if not e_tokens:
            return packets
        ...
        for packet in packets:
            if packet.get('packet_type') == 'table':
                # F7論理データ内のテキストを洗い替え
                f7_logic = packet.get('f7_logic', {})

                # tagged_texts
                for text_item in f7_logic.get('tagged_texts', []):
                    original = text_item.get('text', '')
                    if original:
                        scrubbed = self._find_best_match(original, e_tokens)
                        if scrubbed and scrubbed != original:
                            text_item['text'] = scrubbed
                            text_item['scrubbed_from'] = original
                            scrub_count += 1

                # rows
                for row in f7_logic.get('rows', []):
                    for i, cell in enumerate(row):
                        if cell:
                            scrubbed = self._find_best_match(cell, e_tokens)
                            if scrubbed and scrubbed != cell:
                                row[i] = scrubbed
                                scrub_count += 1

                # headers
                for i, header in enumerate(f7_logic.get('headers', [])):
                    if header:
                        scrubbed = self._find_best_match(header, e_tokens)
                        if scrubbed and scrubbed != header:
                            f7_logic['headers'][i] = scrubbed
                            scrub_count += 1

            elif packet.get('packet_type') == 'text':
                # テキストパケットの洗い替え
                original = packet.get('text', '')
                if original:
                    # テキスト全体ではなく、単語単位で洗い替え
                    words = original.split()
                    scrubbed_words = []
                    for word in words:
                        scrubbed = self._find_best_match(word, e_tokens)
                        scrubbed_words.append(scrubbed if scrubbed else word)
                    packet['text'] = ' '.join(scrubbed_words)

        if scrub_count > 0:
            logger.info(f"[G-Gate] 洗い替え: {scrub_count}件置換")

        return packets

    def _find_best_match(self, ocr_text: str, e_tokens: set) -> Optional[str]:
        """
        OCRテキストに最も類似するEトークンを探す
        """
        ...
        # 完全一致
        if ocr_clean in e_tokens:
            return ocr_clean

        # 編集距離ベースのマッチング
        best_match = None
        best_distance = float('inf')
        max_distance = max(1, len(ocr_clean) // 4)  # 25%許容

        for e_token in e_tokens:
            if abs(len(e_token) - len(ocr_clean)) > max_distance:
                continue

            distance = self._edit_distance(ocr_clean, e_token)
    ...
    def _build_g1_relay(
        ...
        for packet in table_packets:
            ...
            table = {
                ...
                'title': f7_logic.get('title', ''),
                'table_type': f7_logic.get('table_type', 'visual_table'),
                'headers': f7_logic.get('headers', []),
                'rows': f7_logic.get('rows', []),
                'row_count': f7_logic.get('row_count', 0),
                'col_count': f7_logic.get('col_count', 0),
                # Ver 5.7: 三位一体データ
                'x_headers': f7_logic.get('x_headers', []),
                'y_headers': f7_logic.get('y_headers', []),
                'tagged_texts': f7_logic.get('tagged_texts', []),
                'f8_anchors': packet.get('f8_anchors', []),
                'source': packet.get('source', 'f7'),
                'is_heavy': f7_logic.get('row_count', 0) >= 20,
            }
            tables.append(table)
        ...
        return {
            'tables': tables,
            'table_page_context': {'e_content': e_content},
            'table_count': len(tables),
            'heavy_table_count': sum(1 for t in tables if t.get('is_heavy', False)),
            'version': '5.7'
        }
...
class StageH1Table:
    ...
    def process(
        ...
        for table in table_inventory:
            ...
            # ============================================
            # 【Ver 5.6】tagged_texts形式の処理
            # ============================================
            tagged_texts = table.get('tagged_texts', [])
            if tagged_texts:
                logger.info(f"[Stage H1] Ver5.6形式検出: {ref_id} - tagged_texts={len(tagged_texts)}")
                ...
                if built_table and built_table.get('rows'):
                    ...
                    removed_table_ids.add(ref_id)
                    stats['processed'] += 1
                    logger.info(f"[Stage H1] Ver5.6テーブル構築完了: {ref_id} ({len(processed_table['rows'])}行)")
                    continue  # Ver 5.6形式で処理完了、次の表へ
    ...
    def build_table_from_tagged_texts(
        ...
        """
        【Ver 5.0】ヘッダータグ付きテキストからテーブルを構築

        グリッド思考を完全廃止。
        各テキストの x_header/y_header を軸に交差点を逆引きして表を構築。

        Args:
            tagged_texts: F-7からのタグ付きテキストリスト
                [{text, x_header, y_header, id, ...}, ...]
            x_headers: X軸ヘッダーリスト（省略時は自動抽出）
            y_headers: Y軸ヘッダーリスト（省略時は自動抽出）

        Returns:
            {
                'columns': ['y_header', 'x_header1', 'x_header2', ...],
                'rows': [[y1, val11, val12, ...], [y2, val21, val22, ...], ...],
                'row_count': int,
                'col_count': int,
                'source': 'ver5_tagged'
            }
        """
        logger.info(f"[Stage H1] Ver5.0テーブル構築開始: {len(tagged_texts)}テキスト")

        # タグ付きテキストのみ抽出（type=taggedまたはx_header/y_headerが存在）
        valid_texts = [
            t for t in tagged_texts
            if t.get("x_header") and t.get("y_header")
        ]

        if not valid_texts:
            logger.warning("[Stage H1] Ver5.0: タグ付きテキストなし")
            return {
                'columns': [],
                'rows': [],
                'row_count': 0,
        ...
        logger.info(f"[Stage H1] Ver5.0: X軸ヘッダー{len(x_headers)}個, Y軸ヘッダー{len(y_headers)}個")

        # 交差点マップを構築: {(y_header, x_header): [texts]}
        intersection_map = {}
        for text in valid_texts:
            x_h = text.get("x_header", "")
            y_h = text.get("y_header", "")
            key = (y_h, x_h)
            if key not in intersection_map:
                intersection_map[key] = []
            intersection_map[key].append(text.get("text", ""))
        ...
        logger.info(f"[Stage H1] Ver5.0テーブル構築完了: {len(rows)}行 x {len(columns)}列")
