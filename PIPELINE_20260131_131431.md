...
Generated: 2026-01-31T13:14:31.510329
Files: 32
...
- shared/pipeline/stage_f_visual.py (3825 lines)
- shared/pipeline/stage_g1_table_refiner.py (1183 lines)
- shared/pipeline/stage_g2_text_refiner.py (919 lines)
- shared/pipeline/stage_g_gate.py (336 lines)
- shared/pipeline/stage_g_refiner.py (816 lines)
...
- shared/ai/llm_client/llm_client.py (667 lines)
...
【設計 2026-01-31】Ver 6.2 物理座標ID溶接モード
...
F-6〜F-8: 独立・二重読解（AIの本番）
  ...
  - F-7: Dual Read - Path A (テキストの鬼 / 2.5 Flash-Lite)
        【Ver 6.1】CSV型JSON出力で70%トークン削減
...
【Ver 6.1 変更点】
  - F-7プロンプト: cols + rows 形式（値のみ配列）に換装
  - パース処理: CSV型JSON → tagged_texts 自動展開
  - トークン削減: 1校あたり約100文字 → 30文字以下
============================================
"""
import json
import time
...
class StageFVisualAnalyzer:
    ...
    def _process_image_document(
        ...
        for chunk_idx, chunk_pages in enumerate(page_chunks):
            ...
            try:
                ...
                logger.info(f"[F-8→F-7] ヘッダー情報: X={len(f8_headers.get('x_headers', []))}個, Y={len(f8_headers.get('y_headers', []))}個")
            ...
            # Step 2: F-7 後行（文字読み取り + F-8のヘッダーで機械的転記）
            if progress_callback:
            ...
            try:
                path_a_result = self._f7_path_a_chunk_extraction_v62(
                    chunk_pages, classified_blocks, chunk_idx, chunk_start_page,
                    page_column_boundaries, f8_headers
                )
                logger.info(f"[F-7] チャンク{chunk_idx + 1} 文字転記完了")
    ...
    def _build_f7_prompt(self) -> str:
        """
        【Ver 6.3】AIは文字を読むだけ。位置判断は一切させない。

        AIの仕事: 黄色ラベル(#ID)の横にある文字を読み取るだけ
        プログラムの仕事: Surya座標から最短距離でヘッダーを決定
        """
        return """# テキスト読み取り専用指示（Ver 6.3）

## あなたの任務
画像内の「黄色ラベル（#ID）」が付与された各領域の**文字だけを読み取れ**。
位置関係の判断、ヘッダーへの紐付けは**一切行うな**。それはプログラムの仕事だ。
...
### 形式の説明
- `texts`: 配列の配列。各要素は `[ID, テキスト]`
- `ID`: 黄色ラベルに書かれた数字（#を除く）
- `テキスト`: そのラベルの横にある文字を正確に読み取った内容

## 重要なルール
1. **全テキスト抽出**: 黄色ラベル付きの全領域を漏れなく読み取れ
2. **省略禁止**: 似た内容でも全て個別に出力
3. **位置判断禁止**: どれがヘッダーか、どこに属するかは考えるな
4. **シンプルに**: `[ID, テキスト]` の配列だけを出力せよ

## 禁止事項
- ヘッダーの特定や分類
- 位置関係の推論
- データの要約や省略
- 余計なフィールドの追加（xh, yh, type など禁止）
"""
...
"""
    ...
    def _f7_path_a_chunk_extraction_v62(
        ...
        f8_headers: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        【Ver 6.2】F-7: 文字読み取り + プログラムによる座標マッピング
        ...
        Returns:
            抽出結果（tagged_texts 含む）
        """
        import tempfile
        f7_start = time.time()
        logger.info(f"[F-7] 【Ver 6.2】チャンク{chunk_idx + 1} 文字読み取り開始")
        ...
        f8_headers = f8_headers or {'x_headers': [], 'y_headers': [], 'header_coords': {}}

        # ID焼き込み画像を生成
        id_burned_image = self._f6_burn_ids_to_image(original_image, blocks, chunk_start_page)

        # ブロックの座標マップを構築（プログラムによる座標決定用）
        block_coords = {}
        for block in blocks:
            block_id = block.get('block_id', '')
            bbox = block.get('bbox')
        ...
        # 一時ファイルに保存してAIに渡す
        with tempfile.NamedTemporaryFile(suffix=f'_chunk{chunk_idx}_v62.png', delete=False) as f:
            id_burned_image.save(f, format='PNG')
            temp_path = Path(f.name)

        try:
            # Ver 6.2 プロンプト: AIは文字を読むだけ
            prompt = self._build_f7_prompt()
            ...
            f7_elapsed = time.time() - f7_start
            logger.info(f"[F-7] 【Ver 6.2】AIテキスト読み取り完了: {f7_elapsed:.2f}秒")

            # ============================================
            # 【Ver 6.2 核心】プログラムによる座標マッピング
            # AIが読んだ [ID, テキスト] を、Surya座標で最短距離のヘッダーに紐付け
            # ============================================
            tagged_texts = self._map_texts_to_headers_by_coords(
                ai_texts=result.get('texts', []),
                block_coords=block_coords,
                f8_headers=f8_headers
            )

            # 結果を構築
            result['tagged_texts'] = tagged_texts
            result['x_headers'] = f8_headers.get('x_headers', [])
            result['y_headers'] = f8_headers.get('y_headers', [])
            result['_v62_coord_mapping'] = True

            logger.info(f"[F-7] 【Ver 6.2】座標マッピング完了: {len(tagged_texts)}件")

            return result

        except Exception as e:
            logger.error(f"[F-7] 【Ver 6.2】エラー: {e}")
    ...
    def _f7_path_a_chunk_extraction(
        ...
        for patch_idx, patch in enumerate(patches):
            ...
            try:
                ...
                # ============================================
                # 【Ver 6.2】CSV型JSON形式の展開（ID:テキスト溶接対応）
    ...
    def _build_f7_smart_prompt(
        ...
        l_shape_instruction = ""
        if is_continuation:
            l_shape_instruction = f"""
## ⚠️ L字型パッチ構造
- これはパッチ {patch_index + 1}/{total_patches} です
- **上端**: X軸ヘッダー（日付や項目名）が表示されています
- **左端**: Y軸ヘッダー（数値や行項目）が表示されています
- **中央**: 実際のデータ（学校名等）が表示されています

### 重要
- 各テキストの「真上」にあるX軸ヘッダーを特定してください
- 各テキストの「真左」にあるY軸ヘッダーを特定してください
- ヘッダーは重複して出力してOKです（各パッチで独立）
...
- 詳細: {patch_info}
"""
        return base_prompt + l_shape_instruction + context
...
{{
  "rescued": [
    {{"id": "22", "x_header": "2/1", "y_header": "74"}},
    ...
  ]
}}
```

画像上の罫線、文字の並び、物理的な位置関係から判断してください。
"""
    ...
    def _f10_stage_e_scrubbing(
        ...
        """
        【Ver 6.2】F-10: Stage E（pdfplumber）による最終洗い替え

        F-9/F-9.5 で確定した住所タグを保持したまま、
        OCR文字をStage Eの正確なデジタル文字で置換する。

        Args:
            merged_result: F-9 の出力
            e_content: Stage E のテキスト
            post_body: 投稿本文
        ...
        """
        f10_start = time.time()
        logger.info("[F-10] 【Ver 6.2】Stage E洗い替え開始")
        ...
        if e_content:
            ...
            logger.info(f"[F-10] Stage E候補: {len(e_words)}単語")
            ...
            for tt in tagged_texts:
                ...
                if best_match and best_match != ocr_text:
                    ...
                    tt['_scrubbed'] = True
                    scrub_count += 1
                    logger.debug(f"[F-10洗替] '{ocr_text}' → '{best_match}'")

        logger.info(f"[F-10] 洗い替え完了: {scrub_count}/{len(tagged_texts)}件")
        ...
        payload = {
            ...
            "metadata": {
                **merged_result.get("metadata", {}),
                "f10_scrub_count": scrub_count,
                "f10_elapsed": f10_elapsed
            },
            "warnings": []
        }

        logger.info(f"[F-10] 完了: {f10_elapsed:.2f}秒")
...
```python
"""
Stage G1: Table Refiner（表専用整理）- Ver 6.2 物理分離対応

【設計 2026-01-31】Ver 6.2: 物理分離フロー

役割: F-10 で洗い替え済みの「表データ（type: cell）」のみを受け取り、
      セル内の文字を溶接（校閲）して H1 用 JSON を出力

============================================
【Ver 6.2】物理分離フロー:
  - F-10 で洗い替え完了済み（Stage E の正確な文字）
  - G1 は「表(cell)」のみを受け取る（地文は G2 へ）
  - セル内の文字溶接（分断された単語の結合）に専念

入力:
  - tables: 表データリスト（tagged_texts 含む）
  - e_content: Stage E テキスト（参照用）
  - _v62_cell_only: True（Ver 6.2 フラグ）

出力:
  - tables: 溶接済み表データ（H1 へ直送）
  - token_usage: トークン使用量
============================================
"""
import json
...
class StageG1TableRefiner:
    ...
    def __init__(self, llm_client=None):
        ...
        self._token_usage: Dict[str, Any] = {
            ...
            'model': G1_MODEL,
            'polished_tables': 0,
            'e_replacements': 0  # 【Ver 5.9】E置換カウント
        }

    def process(self, g1_input: Dict[str, Any]) -> Dict[str, Any]:
        """
        表データを検証・整形（Ver 5.9 座標ベースE置換対応）

        Args:
            g1_input: G-Gate からの入力
                - tables: 表データリスト
                - e_content: Stage Eの正確なテキスト（Ver 5.9）
                - f8_anchors: Suryaの物理座標（Ver 5.9）
        ...
        # 以下は従来処理（Ver 5.9 以前）
        logger.info("[G1] 表整理開始（Ver 5.9 座標ベースE置換対応）...")

        tables = g1_input.get('tables', [])
        e_content = g1_input.get('e_content', '')  # 【Ver 5.9】Stage Eテキスト
        f8_anchors = g1_input.get('f8_anchors', [])  # 【Ver 5.9】Surya座標
        ...
        fixed_count = 0
        polished_count = 0
        e_replaced_count = 0  # 【Ver 5.9】E置換カウント
        ...
        for table in tables:
            ...
            # Step 3: 【Ver 5.9】座標ベースE置換（AIの前に実行）
            # 物理座標を使ってOCRテキストをStage Eの正確なテキストに置換
            # これにより「筑駒」→「筑波大学附属駒場」などの誤読を物理的に修正
            table_f8 = table.get('f8_anchors', f8_anchors)  # 表固有のf8があれば優先
            if e_content and table_f8:
                replaced_table, replace_count = self._replace_with_e_text_by_coords(
                    table, e_content, table_f8
                )
                if replace_count > 0:
                    table = replaced_table
                    e_replaced_count += replace_count
                    self._token_usage['e_replacements'] += replace_count

            # Step 4: AI研磨（全件実行 - 常駐化）
            ...
            # Step 5: 最終整形（Ver 5.9: e_contentを渡す）
            formatted_table = self._format_table_for_h1(table, validation, e_content)
        ...
        statistics = {
            ...
            'e_replaced_count': e_replaced_count,  # 【Ver 5.9】E置換カウント
        ...
        logger.info(f"[G1] 【Ver 5.9】E置換: {e_replaced_count}件")
        logger.info(f"[G1] AI研磨: {polished_count}表に適用")

        logger.info(f"[G1] 完了: {valid_count}/{len(tables)}表が有効, "
                   f"{fixed_count}表を自動修正, {e_replaced_count}件E置換, {polished_count}表をAI研磨, 計{total_rows}行")
    ...
    def _process_v62(self, g1_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            H1 用の整形済み JSON
        """
        logger.info("[G1] 【Ver 6.2】表専用レーン処理開始")
        ...
        for table in tables:
            ...
            # セル内文字の溶接（AI使用）
            if self.llm and tagged_texts:
                welded_texts, weld_count = self._weld_cell_texts_v62(
                    tagged_texts, x_headers, y_headers
                )
                total_welded += weld_count
            else:
        ...
        logger.info(f"[G1] 【Ver 6.2】完了: {len(processed_tables)}表, 溶接{total_welded}件")
    ...
    def _weld_cell_texts_v62(
        ...
        """
        【Ver 6.2】セル内文字の溶接（AI使用）

        分断された単語を文脈から判断して結合。
        例: 「筑波大」「学附属」「駒場」→「筑波大学附属駒場」

        Args:
            tagged_texts: セルデータリスト
            x_headers: X軸ヘッダー
            y_headers: Y軸ヘッダー
        ...
        Returns:
            (溶接済みリスト, 溶接件数)
        """
        if not tagged_texts or len(tagged_texts) < 5:
            # 少量なら溶接不要
            return tagged_texts, 0

        # 同一セル（同じ xh, yh）のテキストをグループ化
        cell_groups = {}
        for tt in tagged_texts:
            key = (tt.get('x_header', ''), tt.get('y_header', ''))
            if key not in cell_groups:
                cell_groups[key] = []
            cell_groups[key].append(tt)

        # 複数テキストを持つセルのみ溶接対象
        weld_targets = {k: v for k, v in cell_groups.items() if len(v) > 1}

        if not weld_targets:
            return tagged_texts, 0

        # AI に溶接を依頼
        weld_prompt = f"""# セル内文字の溶接（Ver 6.2）

以下のセルには複数の文字断片があります。これらを1つの正しい単語/文に結合してください。

## 溶接対象
{json.dumps({str(k): [t.get('text', '') for t in v] for k, v in weld_targets.items()}, ensure_ascii=False, indent=2)}
...
"""
        ...
        try:
            ...
            for tt in tagged_texts:
                key = str((tt.get('x_header', ''), tt.get('y_header', '')))
                if key in welded_map:
                    # 溶接済み
                    new_tt = tt.copy()
                    new_tt['text'] = welded_map[key]
                    new_tt['_welded'] = True
                    welded_texts.append(new_tt)
                    weld_count += 1
                    # グループの最初のアイテムだけ残す
                    del welded_map[key]
                elif (tt.get('x_header', ''), tt.get('y_header', '')) not in weld_targets:
                    # 溶接対象外
                    welded_texts.append(tt)

            logger.debug(f"[G1] 【Ver 6.2】溶接完了: {weld_count}セル")
            return welded_texts, weld_count

        except Exception as e:
            logger.warning(f"[G1] 【Ver 6.2】溶接失敗: {e}")
            return tagged_texts, 0

    # ============================================
    # 【Ver 5.9】座標ベースのStage E置換
    # ============================================
    def _replace_with_e_text_by_coords(
        self,
        table: Dict[str, Any],
        e_content: str,
        ...
        AIに頼らず、物理座標を使ってOCRテキストをStage Eの正確なテキストに置換。
        「筑駒」→「筑波大学附属駒場」のような誤読を物理的に修正。

        アルゴリズム:
        1. f8_anchorsから座標→OCRテキストのマップを構築
        2. e_contentから正確なテキストのリストを抽出
        3. 座標が近いテキスト同士をマッチング
        4. OCRテキストをE正確テキストで置換

        Args:
            table: 置換対象の表
            e_content: Stage Eの正確なテキスト
            f8_anchors: Suryaの物理座標リスト

        Returns:
            (置換後の表, 置換件数)
        """
        if not e_content or not f8_anchors:
            return table, 0

        anchor_id = table.get('anchor_id', 'unknown')
        logger.info(f"[G1] 【Ver 5.9】座標ベースE置換開始: {anchor_id}")

        # ============================================
        # Step 1: 座標マップを構築
        # ============================================
        coord_to_ocr = {}  # {(x, y): ocr_text}
        for anchor in f8_anchors:
            ...
            if text and bbox:
                if isinstance(bbox, dict):
                    ...
                    continue
                coord_to_ocr[(x_center, y_center)] = text

        if not coord_to_ocr:
            logger.debug(f"[G1] E置換: 座標マップなし")
            return table, 0

        # ============================================
        # Step 2: E正確テキストから学校名候補を抽出
        # ============================================
        # 改行・空白で分割して候補リストを作成
        e_candidates = []
        for line in e_content.split('\n'):
            ...
            if line and len(line) >= 2:
                # 短すぎる文字列は除外（ノイズ防止）
                e_candidates.append(line)
                # 空白で分割した断片も候補に
                for word in line.split():
                    if len(word) >= 2:
                        e_candidates.append(word)
        ...
        for c in e_candidates:
            if c not in seen:
                seen.add(c)
                e_candidates_unique.append(c)
        e_candidates = e_candidates_unique

        logger.debug(f"[G1] E候補テキスト: {len(e_candidates)}個")

        # ============================================
        # Step 3: OCRテキストをE正確テキストに置換
        # ============================================
        replacement_count = 0
        replacement_map = {}  # {ocr_text: e_correct_text}

        for (x, y), ocr_text in coord_to_ocr.items():
            # OCRテキストに類似するE正確テキストを探す
            best_match = None
            best_score = 0

            for e_text in e_candidates:
                # 類似度スコアを計算（部分一致 + 長さ考慮）
                score = self._calc_similarity_score(ocr_text, e_text)
                if score > best_score and score >= 0.5:  # 50%以上の類似度
                    best_score = score
                    best_match = e_text

            if best_match and best_match != ocr_text:
                replacement_map[ocr_text] = best_match
                logger.debug(f"[G1-E置換] '{ocr_text}' → '{best_match}' (類似度={best_score:.2f})")

        # ============================================
        # Step 4: テーブル内のテキストを置換
        # ============================================
        if not replacement_map:
            logger.debug(f"[G1] E置換: 該当なし")
            return table, 0

        replaced_table = table.copy()

        # tagged_textsを置換
        if 'tagged_texts' in replaced_table:
            new_tagged_texts = []
            for tt in replaced_table['tagged_texts']:
                new_tt = tt.copy()
                text = tt.get('text', '')
                if text in replacement_map:
                    new_tt['text'] = replacement_map[text]
                    new_tt['original_ocr'] = text  # 元のOCRテキストを保持
                    replacement_count += 1
                new_tagged_texts.append(new_tt)
            replaced_table['tagged_texts'] = new_tagged_texts

        # rowsを置換
        if 'rows' in replaced_table:
            new_rows = []
            for row in replaced_table['rows']:
                if isinstance(row, list):
                    new_row = []
                    for cell in row:
                        cell_str = str(cell)
                        replaced_cell = cell_str
                        for ocr, correct in replacement_map.items():
                            if ocr in cell_str:
                                replaced_cell = replaced_cell.replace(ocr, correct)
                                if ocr != replaced_cell:
                                    replacement_count += 1
                        new_row.append(replaced_cell)
                    new_rows.append(new_row)
                else:
                    new_rows.append(row)
            replaced_table['rows'] = new_rows

        replaced_table['e_replacement_count'] = replacement_count
        logger.info(f"[G1] 【Ver 5.9】E置換完了: {anchor_id} - {replacement_count}件置換")

        return replaced_table, replacement_count

    def _calc_similarity_score(self, ocr_text: str, e_text: str) -> float:
        """
        OCRテキストとE正確テキストの類似度スコアを計算
        ...
        # 部分一致チェック
        if ocr_text in e_text:
            # OCRが正確テキストの一部（例: "筑駒" in "筑波大学附属駒場"）
            return 0.8 * (len(ocr_text) / len(e_text))

        if e_text in ocr_text:
            # 正確テキストがOCRの一部（珍しいケース）
            return 0.7

        # 文字の重複率
        ocr_chars = set(ocr_text)
        e_chars = set(e_text)
        if not ocr_chars or not e_chars:
            return 0.0

        intersection = ocr_chars & e_chars
        union = ocr_chars | e_chars
    ...
    def _polish_table_with_ai(
        ...
        try:
            ...
            # 【Ver 5.8】Fail-Fast: サイレントフォールバック禁止
            # AI研磨に失敗した不完全なデータをH1へ流すことを物理的に禁止
            error_msg = f"[G1] AI研磨失敗（Fail-Fast）: {anchor_id} - {e}"
            logger.error(error_msg)
...
"""
    ...
    def _format_table_for_h1(
        ...
        return {
            ...
            # 【Ver 5.9】三位一体データ + E正確テキストを直送
            'x_headers': table.get('x_headers', []),
            'y_headers': table.get('y_headers', []),
            'tagged_texts': table.get('tagged_texts', []),
            'f8_anchors': table.get('f8_anchors', []),
            'e_content': e_content,  # 【Ver 5.9】E正確テキスト
            'e_replacement_count': table.get('e_replacement_count', 0),
        }
...
```python
"""
Stage G2: Text Refiner（テキスト専用整理）- Ver 6.2 物理分離対応

【設計 2026-01-31】Ver 6.2: 物理分離フロー

役割: F-10 で洗い替え済みの「地文（text）」のみを受け取り、
      AI で一本の完璧な原稿に繋ぎ直す

============================================
【Ver 6.2】物理分離フロー:
  - F-10 で洗い替え完了済み（Stage E の正確な文字）
  - G2 は「地文(text)」のみを受け取る（表は G1 へ）
  - 表アンカー [→ TBL_xxx 参照] を適切な位置に保持

入力:
  - text_blocks: 地文データリスト（F-10 洗い替え済み）
  - table_anchors: 表のアンカー情報
  - post_body: 投稿本文
  - _v62_text_only: True（Ver 6.2 フラグ）
...
class StageG2TextRefiner:
    ...
    def _process_v62(self, g2_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.llm and len(raw_text) > 100:
            unified_text = self._unify_text_v62(raw_text, table_anchors)
        else:
            unified_text = raw_text
        ...
        logger.info(f"[G2] 【Ver 6.2】完了: {len(segments)}セグメント, unified_text={len(unified_text)}文字")
    ...
    def _unify_text_v62(
        ...
        """
        【Ver 6.2】AI で断片的なテキストを一本の原稿に統合

        Args:
            raw_text: 結合済みの生テキスト
            table_anchors: 表アンカー情報

        Returns:
            統合済みテキスト
        """
        anchor_ids = [a.get('anchor_id', '') for a in table_anchors]

        prompt = f"""# 地文の一本化（Ver 6.2）

以下の断片的なテキストを、論理的に繋がる「一本の完璧な原稿」に統合してください。

## ルール
1. OCRのゴミ（不自然な改行、謎の記号）を浄化
2. 文脈を考慮して自然な日本語に整形
3. **表アンカー [→ TBL_xxx 参照] は必ず保持**（削除禁止）
4. 内容の追加・削除は禁止（整形のみ）
...
"""
        ...
        try:
            ...
            for anchor_id in anchor_ids:
                if anchor_id and anchor_id not in response:
                    logger.warning(f"[G2] 【Ver 6.2】アンカー消失: {anchor_id}")
                    response += f"\n\n[→ {anchor_id} 参照]"

            logger.info(f"[G2] 【Ver 6.2】AI統合完了: {len(raw_text)}→{len(response)}文字")
            return response.strip()

        except Exception as e:
            logger.warning(f"[G2] 【Ver 6.2】AI統合失敗: {e}")
            return raw_text
...
class StageGRefiner:
    ...
    def _build_direct_inputs(
        ...
        """
        【Ver 6.2】F-10 の出力を type で物理分離して G1/G2 に直結
        ...
        logger.info(f"[Stage G] 【Ver 6.2】物理分離完了:")
        logger.info(f"  ├─ cell(表) → G1: {len(cell_items)}件")
        logger.info(f"  └─ text(地文) → G2: {len(text_items)}件")
        ...
        logger.info(f"[Stage G] 【Ver 6.2】直結入力構築完了:")
        logger.info(f"  ├─ G1: {len(tables)}表 ({len(cell_items)}セル)")
        logger.info(f"  └─ G2: {len(text_items_sorted)}テキスト + {len(table_anchors)}アンカー")
