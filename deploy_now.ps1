...
$result2 = gcloud run deploy doc-processor --image asia-northeast1-docker.pkg.dev/consummate-yew-479020-u2/cloud-run-source-deploy/doc-processor:latest --region asia-northeast1 --allow-unauthenticated --service-account document-management-system@consummate-yew-479020-u2.iam.gserviceaccount.com --timeout 3600 --memory 16Gi --cpu 4 --set-env-vars "GOOGLE_AI_API_KEY=$env:GOOGLE_AI_API_KEY,ANTHROPIC_API_KEY=$env:ANTHROPIC_API_KEY,OPENAI_API_KEY=$env:OPENAI_API_KEY,SUPABASE_URL=$env:SUPABASE_URL,SUPABASE_KEY=$env:SUPABASE_KEY,SUPABASE_SERVICE_ROLE_KEY=$env:SUPABASE_SERVICE_ROLE_KEY,DOC_PROCESSOR_API_KEY=$env:DOC_PROCESSOR_API_KEY,LOG_LEVEL=INFO" 2>&1
Write-Host $result2
