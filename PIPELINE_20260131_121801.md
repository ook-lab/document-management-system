...
Generated: 2026-01-31T12:18:02.165930
Files: 32
...
- shared/pipeline/stage_f_visual.py (3384 lines)
- shared/pipeline/stage_g1_table_refiner.py (993 lines)
- shared/pipeline/stage_g2_text_refiner.py (760 lines)
- shared/pipeline/stage_g_gate.py (336 lines)
- shared/pipeline/stage_g_refiner.py (774 lines)
...
```python
"""
Stage G1: Table Refiner（表専用整理）- Ver 5.9 座標ベース置換対応

【設計 2026-01-31】Ver 5.9: 座標ベースのStage E置換

役割: G-Gate から受け取った表データを検証・整形し、H1 用 JSON を出力
      AIに頼らず、物理座標でStage Eの正確なテキストを逆引き置換

============================================
入力（G-Gate から）:
  - tables: 表データリスト
  - e_content: Stage Eの正確なテキスト（洗い替え用）
  - f8_anchors: Suryaの物理座標（座標マッピング用）
...
処理フロー:
  1. ルールベース検算
  2. 自動修正（セル数調整等）
  3. 【Ver 5.9】座標ベースE置換（AIの前に物理的に正しいテキストへ置換）
  4. AI研磨（必要な表のみ）
  5. 最終検証・整形
============================================
"""
...
class StageGRefiner:
    ...
    def _build_direct_inputs(
        ...
        """
        【Ver 6.2】G-Gate を介さず、F の出力から直接 G1/G2 入力を構築
        ...
        if tagged_texts:
            ...
            tables.append({
                ...
                'y_headers': y_headers,
                'e_content': e_content,
                '_v62_format': path_a.get('_v62_coord_mapping', False)
            })
        ...
        # テキストブロック
        text_blocks = path_a.get('extracted_texts', [])
        ...
        logger.info(f"  ├─ G1: {len(tables)}表, E={len(e_content)}文字")
        logger.info(f"  └─ G2: {len(text_blocks)}ブロック")
