...
Generated: 2026-01-29T16:39:33.846179
Files: 32
...
class StageFVisualAnalyzer:
    ...
    def _f35_filter_and_columnize(self, blocks: List[Dict]) -> List[Dict]:
        ...
        for page_idx, page_blocks in pages.items():
            ...
            # 列境界を検出（空白 OR 罫線）- 最大隙間優先・最大5列
            MIN_GAP_WIDTH = 20  # 最小空白幅
            MIN_LINE_HEIGHT = 300  # 罫線とみなす最小高さ（1000グリッド中）
            MAX_INTERNAL_BOUNDARIES = 4  # 内部境界線の上限（最大5列）
            ...
            while x < QUANTIZE_GRID_SIZE:
                ...
                if x_histogram[x] == 0:
                    ...
                    if gap_width >= MIN_GAP_WIDTH:
                        ...
                        if boundary > 50 and boundary < 950:
                            boundary_candidates.append((boundary, gap_width))
                    continue
                ...
                if x_max_height[x] >= MIN_LINE_HEIGHT and x_histogram[x] <= 3:
                    ...
                    if line_width <= 15:
                        ...
                        if boundary > 50 and boundary < 950:
                            # 罫線は幅が狭いので、重要度として100を加算
                            boundary_candidates.append((boundary, 100))
                    continue

                x += 1

            # 幅が広い順にソートして上位MAX_INTERNAL_BOUNDARIES個を採用
            if len(boundary_candidates) > MAX_INTERNAL_BOUNDARIES:
                original_count = len(boundary_candidates)
                boundary_candidates.sort(key=lambda x: x[1], reverse=True)
                boundary_candidates = boundary_candidates[:MAX_INTERNAL_BOUNDARIES]
                logger.info(f"  ├─ [Safety] 列境界制限: {original_count} → top {MAX_INTERNAL_BOUNDARIES} by gap width")

            # 位置順にソート
            boundary_candidates.sort(key=lambda x: x[0])
            internal_boundaries = [b[0] for b in boundary_candidates]

            # 左右端を追加
            column_boundaries = [0] + internal_boundaries + [QUANTIZE_GRID_SIZE]
...
"""
    ...
    def _smart_crop_patches(
        ...
        """
        Ver 3.1: 水平優先・インデックス列複製・のりしろ付きクロッピング

        Args:
            image: 元画像
            column_boundaries: F-3.5で検出された列境界（量子化座標0-1000）
            overlap: のりしろ幅（ピクセル）

        Returns:
            [{'image': PIL.Image, 'type': 'full'|'h_section'|'v_column', 'info': str}, ...]
        """
        ...
        for i in range(len(h_boundaries) - 1):
            ...
            if y_end - y_start > 50:  # 最小高さチェック
                h_sections.append((y_start, y_end))

        logger.info(f"[SmartCrop] 水平セクション: {len(h_sections)}個")

        # 水平セクションがない（または1つだけ）= 全体を処理
        if len(h_sections) <= 1:
            h_sections = [(0, img_height)]

        # Step 3: 各水平セクションを処理
        for sec_idx, (y_start, y_end) in enumerate(h_sections):
            section_img = image.crop((0, y_start, img_width, y_end))
            section_height = y_end - y_start

            # このセクションが大きい & 垂直分割可能か判定
            num_columns = len(column_boundaries) - 1 if column_boundaries and len(column_boundaries) > 1 else 1

            if num_columns <= 1:
                # 列分割なし → セクション全体を1パッチとして追加
                patches.append({
                    'image': section_img,
                    'type': 'h_section' if len(h_sections) > 1 else 'full',
                    'info': f'セクション{sec_idx + 1}/{len(h_sections)}' if len(h_sections) > 1 else '全体（分割なし）'
                })
            else:
                # 垂直ガターを検出（このセクション内で）
                v_gutters = self._detect_gutters(section_img, direction='vertical', min_gap=30)

                # 垂直ガターが不明確（列境界と一致しない）なら分割しない
                if len(v_gutters) < num_columns - 1:
                    logger.info(f"[SmartCrop] セクション{sec_idx + 1}: 垂直ガター不明確 → 分割せず")
                    patches.append({
                        'image': section_img,
                        'type': 'h_section',
                        'info': f'セクション{sec_idx + 1}（垂直分割見送り）'
                    })
                else:
                    # 垂直分割実行（インデックス列を複製）
                    # 左端の列をインデックス列として扱う
                    index_col_end = int(column_boundaries[1] * img_width / QUANTIZE_GRID_SIZE)
                    index_col_img = section_img.crop((0, 0, index_col_end + overlap, section_height))

                    for col_idx in range(1, num_columns):  # 2列目以降
                        ...
                        col_img = section_img.crop((px_start, 0, px_end, section_height))
    ...
    def _f7_path_a_chunk_extraction(
        ...
        for patch_idx, patch in enumerate(patches):
            ...
            logger.info(f"[F-7] パッチ{patch_idx + 1}/{len(patches)}: {patch_info} ({patch_img.size[0]}x{patch_img.size[1]})")
            ...
            try:
                # パッチ用プロンプト
                prompt = self._build_f7_smart_prompt(block_map, chunk_idx, chunk_start_page, patch_info, patch_type, len(patches))
    ...
    def _build_f7_smart_prompt(self, block_map: str, chunk_idx: int, start_page: int, patch_info: str, patch_type: str, total_patches: int) -> str:
        """F-7 SmartCrop用プロンプト構築"""
...
"""
        elif patch_type == 'v_column_with_index':
            context = f"""
## 画像情報（インデックス列付き垂直分割）
- {patch_info}
- ページ番号: {start_page + 1}ページ目
- **左端にインデックス列（偏差値・時限等）が複製されています**
- インデックス列を基準に、右側のデータを正確に読み取ってください
"""
        else:
            context = f"""
## 画像情報
