...
Generated: 2026-01-28T05:24:20.321296
Files: 29
...
- shared/pipeline/pipeline.py (954 lines)
...
- shared/pipeline/stage_g_refiner.py (853 lines)
...
class StageGRefiner:
    ...
    def _llm_merge(
        ...
        if hasattr(self.llm, 'last_usage') and self.llm.last_usage:
            self._g_usage = self.llm.last_usage.copy()
            self._g_usage['model'] = model
            logger.info(f"[Stage G] トークン使用量: prompt={self._g_usage.get('prompt_tokens', 0)}, completion={self._g_usage.get('completion_tokens', 0)}")
...
- 表の一部だけを抽出することは禁止です。"""
    ...
    def _normalize_llm_result(
        ...
        return {
            ...
            'post_body': post_body or {},
            'llm_usage': {'G': self._g_usage} if self._g_usage else {}
        }
