...
Generated: 2026-01-28T09:58:33.108380
Files: 32
...
- shared/pipeline/constants.py (72 lines)
...
- shared/pipeline/pipeline.py (956 lines)
- shared/pipeline/prompts/__init__.py (171 lines)
- shared/pipeline/stage_e_preprocessing.py (344 lines)
- shared/pipeline/stage_f_visual.py (1739 lines)
- shared/pipeline/stage_g1_table_refiner.py (460 lines)
- shared/pipeline/stage_g2_text_refiner.py (441 lines)
- shared/pipeline/stage_g_gate.py (623 lines)
- shared/pipeline/stage_g_refiner.py (688 lines)
- shared/pipeline/stage_h1_table.py (371 lines)
- shared/pipeline/stage_h2_text.py (482 lines)
- shared/pipeline/stage_h_kakeibo.py (617 lines)
- shared/pipeline/stage_hi_combined.py (431 lines)
    ...
    def _f7_path_a_chunk_extraction(
        ...
        try:
            ...
            if hasattr(self.llm_client, 'last_usage') and self.llm_client.last_usage:
                ...
                logger.info(f"[F-7] トークン使用量: prompt={usage.get('prompt_tokens', 0)}, completion={usage.get('completion_tokens', 0)}")
...
```python
"""
Stage G1: Table Refiner（表専用整理）

【設計 2026-01-28】表データの統合・検算・整形
...
class StageG1TableRefiner:
    ...
    def process(self, g1_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"[G1] 完了: {valid_count}/{len(tables)}表が有効, {fixed_count}表を自動修正, 計{total_rows}行")
    ...
    def _validate_table(self, table: Dict[str, Any]) -> TableValidationResult:
        ...
        if headers:
            ...
            for i, row in enumerate(rows):
                if isinstance(row, list) and len(row) != header_count:
                    warnings.append(f"Row {i}: cell count mismatch (expected {header_count}, got {len(row)})")
        ...
        for i, row in enumerate(rows):
            if isinstance(row, list) and all(not str(cell).strip() for cell in row):
                empty_rows.append(i)
        if empty_rows:
            warnings.append(f"Empty rows detected: {empty_rows}")
        ...
        if headers and rows:
            ...
            if empty_cols:
                warnings.append(f"Empty columns detected: {[headers[i] if i < len(headers) else i for i in empty_cols]}")
        ...
        if rows:
            ...
            if duplicates:
                ...
                if dup_ratio > self.MAX_DUPLICATE_ROW_RATIO:
                    ...
                    warnings.append(f"Duplicate rows detected: {len(duplicates)} pairs")
        ...
        if declared_row_count != actual_row_count:
            errors.append(f"Row count mismatch: declared={declared_row_count}, actual={actual_row_count}")

        if declared_col_count != actual_col_count and declared_col_count > 0:
            errors.append(f"Column count mismatch: declared={declared_col_count}, actual={actual_col_count}")
    ...
    def _auto_fix_table(
        ...
        if target_col_count > 0:
            ...
            if fixed_rows != rows:
                fix_descriptions.append("Normalized row cell counts")
    ...
    def structure_from_e_text(
        ...
        Args:
            raw_text: タブ区切りや罫線形式のテキスト
            context: 表周辺のテキスト（タイトル推測用）
...
```python
"""
Stage G2: Text Refiner（テキスト専用整理）

【設計 2026-01-28】テキストの統合・ソート・整形
...
出力（H2 へ）:
  ...
  - unified_text: 統合テキスト（表の跡地にプレースホルダー付き）
  - dedup_stats: 重複排除統計
...
class StageGRefiner:
    ...
    def __init__(self, llm_client: LLMClient):
        ...
        self.g1 = StageG1TableRefiner()
        self.g2 = StageG2TextRefiner()
