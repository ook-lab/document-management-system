-- =========================================
-- SQL関数を source_documents に移行（シンプル版）
-- processing_status条件を削除（3-tier構造対応）
-- =========================================
...
BEGIN
    ...
    WHERE
        -- ✅ processing_status条件を削除（3-tier構造では別テーブル）
        -- 年フィルタ
...
BEGIN
    ...
    WHERE
        ...
        AND d.embedding IS NOT NULL  -- embeddingがあるもののみ
    ORDER BY combined_score DESC
    LIMIT limit_results;
END;
$$ LANGUAGE plpgsql;
...
-- 確認クエリ
-- =========================================
SELECT '✅ All SQL functions migrated to source_documents (simple version)' as status;
