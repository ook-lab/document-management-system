...
-- 【対象】: correction_history テーブル作成, source_documents テーブル拡張
-- 【実行方法】: SupabaseダッシュボードでSQL Editorに貼り付け、実行
-- 【目的】: ユーザーによるメタデータ修正履歴を記録し、ロールバック機能を提供
-- 【修正内容】: documents → source_documents に修正
...
-- ============================================================================
-- Step 3: source_documents テーブルに latest_correction_id カラムを追加
-- ============================================================================

ALTER TABLE source_documents
ADD COLUMN IF NOT EXISTS latest_correction_id BIGINT REFERENCES correction_history(id);

COMMENT ON COLUMN source_documents.latest_correction_id IS
'最新の修正履歴レコードへの参照。ロールバック時に使用。';

-- インデックス作成（NULL許容）
CREATE INDEX IF NOT EXISTS idx_source_documents_latest_correction_id
ON source_documents(latest_correction_id)
...
BEGIN
    ...
    -- source_documentsテーブルを更新（ロールバック）
...
BEGIN
    ...
    RAISE NOTICE '✅ source_documents.latest_correction_id カラムが追加されました';
...
WHERE table_name IN ('correction_history', 'source_documents')
ORDER BY table_name;

-- correction_history のカラム確認
SELECT
    column_name,
    data_type,
    is_nullable
...
SELECT
    ...
    indexdef
FROM pg_indexes
WHERE tablename IN ('correction_history', 'source_documents')
