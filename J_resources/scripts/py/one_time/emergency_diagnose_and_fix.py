...
def main():
    ...
    print("\n[診断1] ドキュメント総数を確認...")
    try:
        result = db.client.table('Rawdata_FILE_AND_MAIL').select('*', count='exact').execute()
    ...
    print("\n[診断3] embedding の有無...")
    try:
        all_docs = db.client.table('Rawdata_FILE_AND_MAIL').select('id,embedding').limit(1000).execute()
    ...
    try:
        result = db.client.table('Rawdata_FILE_AND_MAIL').select('doc_type').execute()
        doc_types = {}
