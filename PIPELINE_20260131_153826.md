...
Generated: 2026-01-31T15:38:26.710712
Files: 32
...
- shared/pipeline/stage_f_visual.py (4068 lines)
- shared/pipeline/stage_g1_table_refiner.py (1276 lines)
- shared/pipeline/stage_g2_text_refiner.py (1216 lines)
...
```python
"""
Stage G1: Table Refiner（表専用整理）- Ver 6.4 物理分離 + 異常再読対応

【設計 2026-01-31】Ver 6.4: e_content完全排除 + 異常セル外科手術

役割:
  1. Fから届いた文字の最終研磨（改行・泣き別れ修正）
  2. F-10 で検出された異常セルのピンポイント再読（外科手術）
...
【Ver 6.4】異常再読フロー:
  - anomaly_report から表セル異常を受け取る
  - 対象座標の画像を切り抜き
  - CLAHE/シャープ化で画質向上
  - Gemini Vision で一点集中読解
  - 結果で元データを上書き
...
```python
"""
Stage G2: Text Refiner（テキスト専用整理）- Ver 6.4 物理分離 + 異常再読対応

【設計 2026-01-31】Ver 6.4: e_content完全排除 + 異常テキスト外科手術

役割:
  1. Fから届いたセグメントの論理的な接続（溶接）
  2. F-10 で検出された異常テキストのピンポイント再読（外科手術）
...
【Ver 6.4】異常再読フロー:
  - anomaly_report からテキスト異常を受け取る
  - 対象座標の画像を切り抜き
  - Gemini Vision で文脈考慮の再読
  - アンカーを破壊しないよう原稿を再溶接
