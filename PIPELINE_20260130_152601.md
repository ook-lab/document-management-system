...
Generated: 2026-01-30T15:26:01.139494
Files: 32
...
- shared/pipeline/stage_f_visual.py (2991 lines)
...
- shared/pipeline/stage_h1_table.py (534 lines)
...
class StageFVisualAnalyzer:
    ...
    def _build_f7_prompt(self) -> str:
        """
        【Ver 5.0】F-7 ヘッダータグ付きテキスト抽出プロンプト

        グリッド思考を完全廃止。
        各テキストに「どのX軸ヘッダー」「どのY軸ヘッダー」の下にあるかをタグ付けする。
        """
        return """# テキスト抽出指示（Ver 5.0）

## あなたのタスク
この画像から全てのテキストを抽出し、各テキストが「どのヘッダーの下にあるか」をタグ付けしてください。

## 重要な概念
- **X軸ヘッダー**: 画像の**最上部**にある見出し（日付、カテゴリ名など）
- **Y軸ヘッダー**: 画像の**最左部**にある見出し（数値、項目名など）
- **テキスト**: それ以外の全ての文字列
...
```json
{
  "x_headers": ["2/1午前", "2/1午後", "2/2午前", "..."],
  "y_headers": ["80", "79", "78", "77", "..."],
  "tagged_texts": [
    {
      "id": "#0",
      "text": "開成",
...
## ルール
1. **#ID番号を正確に読み取る**：画像内の黄色背景・赤文字のラベルがIDです
2. **ヘッダー判定**：各テキストの真上にあるX軸ヘッダー、真左にあるY軸ヘッダーを特定
3. **全テキスト抽出**：省略禁止、要約禁止
4. **ヘッダー外のテキスト**：タイトルや注釈は untagged_texts に格納
...
"""
    ...
    def _smart_crop_patches(
        ...
        # ============================================
        # 【Ver 3.9】垂直分割 - L字型十字基準結合
        # 全パッチに「上：見出し」「左：偏差値」を物理固定
        # ============================================
        if split_axis == 'vertical':
            # 量子化座標を実座標に変換
            boundaries_px = [0]
            ...
            # 【診断ログ】全境界線を表示
            logger.info(f"[SmartCrop] ======== 【Ver 3.9】L字型垂直分割 ========")
            ...
            # 【Ver 3.9】十字基準の切り出し
            # ============================================
            # ヘッダー行（上端5%）を切り出し - 日付・項目名
            HEADER_HEIGHT_RATIO = 0.05
            header_height = int(img_height * HEADER_HEIGHT_RATIO)
            header_row = image.crop((0, 0, img_width, header_height + overlap))
            logger.info(f"[SmartCrop] ヘッダー行: y=0〜{header_height}px (全幅)")

            # インデックス列（左端12%）を切り出し - 偏差値
            INDEX_WIDTH_RATIO = 0.12
            index_width = int(img_width * INDEX_WIDTH_RATIO)
            # ヘッダー部分を除いたインデックス列（重複防止）
            index_col_body = image.crop((0, header_height, index_width + overlap, img_height))
            # フルハイトのインデックス列も保持（1列目用）
            index_col_full = image.crop((0, 0, index_width + overlap, img_height))
            logger.info(f"[SmartCrop] インデックス列: x=0〜{index_width}px")
            ...
            for col_idx in range(num_columns):
                ...
                if col_idx == 0:
                    # 最初の列（インデックス列自体）はそのまま
                    col_img = image.crop((crop_start, 0, crop_end, img_height))
                    logger.info(f"[SmartCrop] 列{col_idx + 1}/{num_columns}: x={x_start}〜{x_end} (インデックス列)")

                    patches.append({
                        'image': col_img,
                        'type': 'v_column_index',
                        'info': f'垂直列{col_idx + 1}/{num_columns} (x:{x_start}-{x_end}) [基準列]',
                        'patch_index': col_idx,
                        'total_patches': num_columns,
                        'is_continuation': False
                    })
                else:
                    # 2列目以降: L字型結合（上にヘッダー、左にインデックス）
                    # データ部分を切り出し（ヘッダー行の下から）
                    data_img = image.crop((crop_start, header_height, crop_end, img_height))
                    data_height = img_height - header_height

                    # L字型キャンバスを作成
                    # 幅: インデックス列 + データ列
                    # 高さ: ヘッダー行 + データ行
                    l_width = index_col_body.width + data_img.width
                    l_height = header_row.height + data_img.height

                    l_canvas = Image.new('RGB', (l_width, l_height), (255, 255, 255))

                    # ヘッダー行を上部に配置（インデックス列の幅分オフセット）
                    # ヘッダーの該当部分のみ切り出し
                    header_for_col = header_row.crop((crop_start, 0, crop_end, header_row.height))
                    l_canvas.paste(header_for_col, (index_col_body.width, 0))

                    # インデックス列を左に配置（ヘッダー行の高さ分オフセット）
                    l_canvas.paste(index_col_body, (0, header_row.height))

                    # データ部分を右下に配置
                    l_canvas.paste(data_img, (index_col_body.width, header_row.height))

                    # 左上コーナー（インデックス列のヘッダー部分）
                    corner = image.crop((0, 0, index_width + overlap, header_height + overlap))
                    l_canvas.paste(corner, (0, 0))

                    logger.info(f"[SmartCrop] 列{col_idx + 1}/{num_columns}: x={x_start}〜{x_end} → L字型({l_width}x{l_height})")

                    patches.append({
                        'image': l_canvas,
                        'type': 'v_column_L_shaped',
                        'info': f'垂直列{col_idx + 1}/{num_columns} (x:{x_start}-{x_end}, +header+index)',
                        'patch_index': col_idx,
                        'total_patches': num_columns,
            ...
            logger.info(f"[SmartCrop] ======== L字型垂直分割完了: {len(patches)}パッチ ========")
            return patches
    ...
    def _f7_path_a_chunk_extraction(
        ...
        for patch_idx, patch in enumerate(patches):
            ...
            try:
                ...
                logger.info(f"[F-7] パッチ{patch_idx + 1} Ver5.0収集: {len([t for t in all_extracted_texts if t.get('patch_idx') == patch_idx])}テキスト, x_headers={len(x_headers)}, y_headers={len(y_headers)}")
