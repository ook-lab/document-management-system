...
- **【特に重要】learning_content_list と monthly_schedule_list**:
  - 表内の**全ての行**を抽出してください（一部の行だけを抽出して残りを省略することは絶対に禁止）
  - 例: 学習予定の表に「国語、算数、理科、社会、音楽、図工」の6教科がある場合、**6件すべて**を learning_content_list に含めること
  - 例: 月間予定の表に20日分の予定がある場合、**20件すべて**を monthly_schedule_list に含めること
...
**重要: 情報の欠損・省略は一切禁止です。原文の全量をJSON構造に落とし込んでください。**"""
   ...
   2. **learning_content_list**: 「今週の学習内容」「各教科の予定」などの学習内容表
      ...
      - **例**: 表に6教科ある場合は、learning_content_list の配列に6個のオブジェクトを含めること
   ...
   - 情報の欠損・省略は一切禁止
   - 原文の全量を構造化データに落とし込む
   - 要約・言い換えは厳禁（特に learning_content_list, monthly_schedule_list, text_blocks の content、important_notes、note フィールド）
   - **【配列フィールドの完全抽出】**: learning_content_list, monthly_schedule_list などの配列フィールドは、表内の**全ての行**を抽出すること（一部だけを代表例として抽出し、残りを省略することは絶対に禁止）
