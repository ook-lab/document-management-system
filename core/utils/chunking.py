...
def chunk_document(text: str, chunk_size: int = 800, chunk_overlap: int = 100) -> List[Dict[str, Any]]:
    """
    文書をチャンクに分割する便利関数
...
def chunk_document_parent_child(
    ...
    """
    文書をParent-Child形式でチャンクに分割する便利関数

    Args:
        text: 分割対象のテキスト
        parent_size: 親チャンクの目標サイズ（1000-2000文字推奨）
        child_size: 子チャンクの目標サイズ（200-400文字推奨）
