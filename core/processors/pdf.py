            ...
            for page_num in page_numbers:
                ...
                try:
                    ...
                    result = self.llm_client.transcribe_image(
                        image_path=tmp_path,
                        prompt="""この画像を詳細に解析し、以下の情報をMarkdown形式で書き起こしてください:

【必須情報】
1. **年度・日付情報**: 画像内の年度表記（例: 2025年度、令和7年度）、発行日、対象期間などを最初に明記
2. **ヘッダー情報**: タイトル、発行者、文書番号、版数など
3. **構造化データ**: 表組み、リスト、予定表、スケジュール、時間割などを正確に再現
4. **本文テキスト**: 段落、箇条書き、注釈、補足情報をすべて含める
5. **フッター情報**: ページ番号、連絡先、注意事項など

【重要な指示】
- 左上や目立つ位置にある年度情報を絶対に見逃さない
- 小さな文字や画像化された注釈も含める
- レイアウトや階層構造（見出し・本文の関係）を保持
- 日付は「○月○日」のように具体的に記載
- 表は列と行の関係が分かるようにMarkdown表形式で記載

見たままを忠実に、情報の欠落なく書き起こしてください。"""
                    )
