...
Generated: 2026-01-29T10:56:04.002631
Files: 32
...
- shared/pipeline/stage_f_visual.py (2364 lines)
...
"""
    ...
    def _detect_gutters(self, image: Image.Image, direction: str = 'horizontal', min_gap: int = 30) -> List[int]:
        """
        画像内のガター（空白の道）を検出

        Args:
            image: PIL Image
            direction: 'horizontal'（水平ガター＝縦に切る線）or 'vertical'（垂直ガター＝横に切る線）
            min_gap: 最小ガター幅（ピクセル）
        ...
        if direction == 'horizontal':
            # 水平ガター検出（Y軸方向にスキャン）= ページを上下に分割する線
            height = pixels.shape[0]
            for y in range(min_gap, height - min_gap):
                # この行が「ほぼ白」かどうかチェック（平均輝度 > 250）
                ...
                if row_mean > 250:
                    ...
                    if gap_height >= min_gap:
                        # ガターの中央を記録
                        gutters.append(start + gap_height // 2)
        else:
            # 垂直ガター検出（X軸方向にスキャン）= ページを左右に分割する線
            ...
            while x < width - min_gap:
                ...
                if col_mean > 250:
                    ...
                    if gap_width >= min_gap:
                        gutters.append(start + gap_width // 2)
                else:
                    x += 1
