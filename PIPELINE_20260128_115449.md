...
Generated: 2026-01-28T11:54:49.746966
Files: 32
...
- shared/pipeline/stage_g1_table_refiner.py (698 lines)
- shared/pipeline/stage_g2_text_refiner.py (598 lines)
...
class StageG2TextRefiner:
    ...
    def _polish_text_with_ai(
        ...
        try:
            prompt = self._build_text_polish_prompt(unified_text, post_body_text)

            logger.info(f"[G2] AI研磨実行中... ({len(unified_text)}文字)")
            response = self.llm.call_model(
                prompt=prompt,
                model=G2_MODEL,
                temperature=0.0
            ...
            if hasattr(self.llm, 'last_usage') and self.llm.last_usage:
                ...
                self._token_usage['completion_tokens'] += usage.get('completion_tokens', 0)
                self._token_usage['total_tokens'] += usage.get('total_tokens', 0)
                logger.info(f"[G2] AI研磨トークン: prompt={usage.get('prompt_tokens', 0)}, "
                           f"completion={usage.get('completion_tokens', 0)}")

            if not response.get('success'):
                logger.warning(f"[G2] AI研磨失敗: {response.get('error')}")
                return None

            polished = response.get('content', '').strip()

            # アンカーが保持されているか確認
            original_anchors = set(re.findall(r'\[→\s*TBL_\d+\s*参照\]', unified_text))
            ...
            if original_anchors and not original_anchors.issubset(polished_anchors):
                logger.warning(f"[G2] AI研磨でアンカーが欠落: {original_anchors - polished_anchors}")
                # アンカーが欠落した場合は元のテキストを返す
                return None
    ...
    def _build_text_polish_prompt(
        ...
        """
        # テキストが長すぎる場合は切り詰める（コスト抑制）
        max_chars = 8000
        if len(unified_text) > max_chars:
            unified_text = unified_text[:max_chars] + "\n...(以下省略)"
