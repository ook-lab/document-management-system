...
class RakutenSeiyuProductIngestionPipeline:
    ...
    def __init__(self, rakuten_id: str, password: str, headless: bool = True):
        ...
        self.scraper: Optional[RakutenSeiyuScraperPlaywright] = None
    ...
    def _prepare_product_data(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data = {
            ...
            "current_price_tax_included": price_float,  # 税込み価格（楽天西友は税込表示）
            ...
            # 表示用
            "display_subject": f"{product_name} - 楽天西友",
            "display_sender": "楽天西友ネットスーパー",
...
async def main():
    ...
    pipeline = RakutenSeiyuProductIngestionPipeline(
        cookies_file="rakuten_seiyu_cookies.json"
    )

    # テスト: 野菜カテゴリーの1ページ目のみ
    result = await pipeline.process_category_page(
        category_id="110001",
        page=1
    )

    logger.info(f"処理結果: {json.dumps(result, indent=2, ensure_ascii=False)}")
