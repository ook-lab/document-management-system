...
flyer_productsテーブルに保存する。

処理フロー:
1. flyer_documents から processing_status='pending' のチラシを取得
2. Gemini 2.5 Pro Vision でチラシ画像から商品情報を抽出
3. flyer_products テーブルに商品データを保存
4. flyer_documents の processing_status を 'completed' に更新
"""
import os
import sys
import json
...
class FlyerProcessor:
    ...
    async def get_pending_flyers(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        try:
            result = self.db.client.table('flyer_documents').select('*').eq(
