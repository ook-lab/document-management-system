...
Generated: 2026-01-31T14:45:06.241251
Files: 32
...
"""
    ...
    def _f10_stage_e_scrubbing(
        ...
        Stage E の座標付き文字（physical_chars）を、
        F-8/F-9 で確定したヘッダー座標に基づいて「枠内流し込み」する。
        AIの推測は一切使わず、幾何学的な包含判定のみで決定。
        ...
        """
        f10_start = time.time()
        logger.info("[F-10] 【Ver 6.4】物理座標による証拠流し込み開始")
        ...
        if physical_chars and x_headers and y_headers:
            # ============================================
            # 【核心】物理座標による枠内流し込み
            # ============================================
            logger.info(f"[F-10] 物理証拠: {len(physical_chars)}文字, X軸{len(x_headers)}個, Y軸{len(y_headers)}個")

            # ヘッダーの座標範囲を構築
            header_coords = merged_result.get('header_coords', {})

            # セルごとに文字を収集 {(xh, yh): [chars...]}
            cell_chars = {}
            unassigned_chars = []

            for char_obj in physical_chars:
                c_text = char_obj.get('text', '')
                if not c_text or c_text.isspace():
                    continue

                bbox = char_obj.get('bbox', [0, 0, 0, 0])
                cx = (bbox[0] + bbox[2]) / 2
                cy = (bbox[1] + bbox[3]) / 2

                # 最短距離でヘッダーを決定
                nearest_xh = self._find_nearest_header(cx, x_headers, header_coords, 'x')
                nearest_yh = self._find_nearest_header(cy, y_headers, header_coords, 'y')

                if nearest_xh and nearest_yh:
                    key = (nearest_xh, nearest_yh)
                    if key not in cell_chars:
                        cell_chars[key] = []
                    cell_chars[key].append({
                        'text': c_text,
                        'x': bbox[0],  # 左端座標（ソート用）
                        'bbox': bbox
                    })
                else:
                    unassigned_chars.append(c_text)

            # 各セル内の文字をX座標順に並べて溶接
            welded_texts = []
            for (xh, yh), chars in cell_chars.items():
                # X座標でソート（左から右へ）
                sorted_chars = sorted(chars, key=lambda c: c['x'])
                welded_text = ''.join(c['text'] for c in sorted_chars)
                ...
                welded_texts.append({
                    ...
                    'y_header': yh,
                    'type': 'cell',
                    '_physical_welded': True,
                    '_char_count': len(sorted_chars)
                })

            logger.info(f"[F-10] 物理溶接完了: {len(welded_texts)}セル, 未割当{len(unassigned_chars)}文字")

            # 物理溶接結果で tagged_texts を上書き
            tagged_texts = welded_texts

        else:
            # フォールバック: 従来の類似度マッチング
            logger.info("[F-10] 物理証拠なし → フォールバック（類似度マッチング）")
            scrub_count = self._f10_fallback_scrubbing(tagged_texts, e_content)
            logger.info(f"[F-10] フォールバック完了: {scrub_count}件")
        ...
        payload = {
            ...
            "metadata": {
                **merged_result.get("metadata", {}),
                ...
                "f10_welded_cells": len(tagged_texts),
