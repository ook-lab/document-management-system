...
生成日時: 2026-02-01T08:21:44.219229
---
## サマリー
- プロンプト定義数: 8
- LLM呼び出し箇所: 12
...
### 2. _build_f7_prompt (stage_f_visual.py)
- 種別: function
...
## 最重要ルール
1) 画像に確実に見える文字列だけを出力する。不確実なら出力しない。
2) "texts" は1ページにつき最大2000件まで。2000件に達したらstop_reason="LIMIT_REACHED"で終了。
3) 同じtextが1ページにつき100回以上出現してはいけない。100回に達したらstop_reason="REPEAT_GUARD"で終了。
4) y座標を一定刻みで増やして「表の続きを生成」する行為は禁止。画像の根拠がないのにbboxを規則的に増やして出力してはいけない。
5) bboxは必ず「その文字が存在する場所」を囲む。推測で座標を作らない。
6) 文字列の正規化（学校名の推測、漢字変換、表記ゆれ統合）禁止。見たままを出力。
7) 赤/黄色のIDラベルは無視（読み取り対象外）。
...
### 3. _build_f8_prompt (stage_f_visual.py)
- 種別: function
...
### 4. rescue_prompt (stage_f_visual.py)
- 種別: f-string
- 説明: 動的パラメータ含む
...
### 5. inline_f_prompt_1 (stage_g1_table_refiner.py)
- 種別: inline-f-string
- 説明: インライン定義（動的パラメータ含む）
...
### 6. inline_f_prompt_1 (stage_g2_text_refiner.py)
- 種別: inline-f-string
- 説明: インライン定義（動的パラメータ含む）
...
### 7. inline_f_prompt_1 (stage_h2_text.py)
- 種別: inline-f-string
- 説明: インライン定義（動的パラメータ含む）
...
### 8. inline_f_prompt_1 (stage_hi_combined.py)
- 種別: inline-f-string
- 説明: インライン定義（動的パラメータ含む）
