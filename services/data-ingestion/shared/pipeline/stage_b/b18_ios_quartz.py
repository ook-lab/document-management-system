"""
B-18: iOS Quartz Processor（iOS Quartz PDFContext 由来 PDF 専用）

iPhone/iPad の「共有→PDF」「プリント→PDF」で生成された PDF を処理する。
iOS Quartz PDF の特徴:
- iOS の Quartz エンジンが生成する標準的な PDF
- テキスト選択可能なページが前提（テキスト選択不可ページは A5 が SCAN に分類）
- Web ページ印刷 / メモ / Pages / Numbers 由来が多い
- 縦長（A4 近似）1 カラムが多い
- 改行・インデントが不規則な場合がある（Web ページ由来）
- 行間・余白は内容により大きく異なる

処理方針:
- B3（Word）と同じテキストフロー抽出
- Web 由来は文字密度が低い傾向 → 行クラスタリングの tol を緩めに
- purge は標準テキスト消去
"""
...
class B18IOSQuartzProcessor:
    """B-18: iOS Quartz Processor（iOS Quartz PDFContext 由来 PDF 専用）"""

    # iOS Quartz は行間が広いため行クラスタリング許容誤差を緩く
    LINE_TOL = 3.0
    GAP_MIN_WIDTH = 10.0

    def process(self, file_path: Path, masked_pages=None, log_file=None) -> Dict[str, Any]:
        """
        iOS Quartz 由来 PDF から構造化データを抽出
        ...
        if log_file:
            _sink_id = logger.add(
                ...
                filter=lambda r: "[B-18]" in r["message"],
                level="DEBUG",
                encoding="utf-8",
            )
        try:
    ...
    def _process_impl(self, file_path: Path, masked_pages=None) -> Dict[str, Any]:
        logger.info(f"[B-18] iOS Quartz PDF処理開始: {file_path.name}")

        try:
            import pdfplumber
        except ImportError:
            logger.error("[B-18] pdfplumber がインストールされていません")
            return self._error_result("pdfplumber not installed")

        try:
            with pdfplumber.open(str(file_path)) as pdf:
                logger.info(f"[B-18] ページ数: {len(pdf.pages)}")
                ...
                for page_num, page in enumerate(pdf.pages):
                    if _masked and page_num in _masked:
                        logger.debug(f"[B-18] ページ{page_num+1}: マスク → スキップ")
                        continue

                    logger.info(
                        f"[B-18] ページ{page_num+1}: "
                        f"chars={len(page.chars or [])} images={len(page.images or [])}"
                    )
                ...
                tags = {
                    ...
                    'processor': 'b18_ios_quartz',
                }

                logger.info(f"[B-18] 抽出完了: ブロック={len(logical_blocks)} 表={len(all_tables)}")
                for idx, block in enumerate(logical_blocks):
                    logger.info(f"[B-18] block{idx} (page={block.get('page')}, slice={block.get('slice')}): {block.get('text', '')}")

                purged_pdf_path = self._purge_extracted_text(file_path, all_words, all_tables)
                logger.info(f"[B-18] purged PDF 生成完了: {purged_pdf_path.name}")
    ...
    def _chars_to_text_lines(self, chars: List[Dict]) -> str:
        ...
        for ch in sorted_chars[1:]:
            if abs(ch['top'] - current_top) <= self.LINE_TOL:
    ...
    def _extract_tables(self, page, page_num: int) -> List[Dict[str, Any]]:
        ...
        for idx, table in enumerate(page.find_tables()):
            ...
            logger.info(f"[B-18] Table {idx} (Page {page_num}): {rows}行×{cols}列")
            if data:
                for row_idx, row in enumerate(data):
                    logger.info(f"[B-18] Table {idx} 行{row_idx}: {row}")
            tables.append({
                'page': page_num,
                'index': idx,
                'bbox': table.bbox,
    ...
    def _purge_extracted_text(
        ...
        """標準テキスト消去（iOS Quartz は標準テキスト埋め込み）"""
        try:
            import fitz
        except ImportError:
            logger.error("[B-18] PyMuPDF がインストールされていません")
            return file_path
        ...
        try:
            ...
            logger.info(f"[B-18] フェーズ1: 抽出文字を削除 {deleted_chars}文字")
            ...
            for attempt in range(1, 6):
                ...
                if not remaining_pages:
                    break
                logger.info(f"[B-18] フェーズ2 試行{attempt}: 残存テキストあり {remaining_pages}")
            ...
            purged_pdf_path = purged_dir / f"b18_{file_path.stem}_purged.pdf"
            ...
            if structured_tables:
                try:
                    ...
                    doc2.close()
                    logger.info(f"[B-18] フェーズ3: 表罫線削除 {deleted_table_graphics}表")
                except Exception as e:
                    logger.warning(f"[B-18] 表罫線削除エラー: {e}")

            logger.info(f"[B-18] purged PDF 保存: {purged_pdf_path.name}")
            return purged_pdf_path

        except Exception as e:
            logger.error(f"[B-18] purge エラー: {e}", exc_info=True)
            return file_path
