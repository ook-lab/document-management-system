...
class GoogleDriveConnector:
    ...
    def update_file_content(self, file_id: str, file_path: Union[str, Path], mime_type: Optional[str] = None) -> bool:
        """
        Google Drive上の既存ファイルをIDを変えずに上書き更新

        Args:
            file_id: 更新対象のファイルID
            file_path: ローカルの新しいファイルパス
            mime_type: MIMEタイプ（Noneの場合は自動判定）
        ...
        try:
            file_path = Path(file_path)
            if not file_path.exists():
                logger.error(f"更新用ファイルが見つかりません: {file_path}")
                return False
            ...
            logger.info(f"ファイル上書き成功: ID={file_id}")
            return True
        except Exception as e:
            logger.error(f"ファイル上書きエラー (ID={file_id}): {e}")
            return False
