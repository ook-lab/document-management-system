...
def extract_page(file_id, page_index):
    try:
        ...
        提供された画像からすべての情報を抽出し、クリーンで構造化されたMarkdown形式で返してください。

        【思考プロセス（重要）】
        正確な抽出のために、以下の手順で思考してください。
        1. **全体俯瞰**: まず画像全体のタイトルや注釈を特定し、文書の目的を把握します。
        2. **逆引き解析 (Bottom-up/Right-to-left)**: 表のデータ行において、まず右側や下側の「詳細セル（具体的な科目、数値、時間など）」に着目し、それらが左側や上側のどの「親項目（学年、校舎、ヘッダー項目）」に属しているかを視覚的な位置関係から逆算して特定してください。
        3. **論理展開**: 結合セルによって視覚的に省略されている親項目の情報を、すべての行に等しく展開して補完します。これにより、1つの行だけでデータが完結するようにします。

        【抽出ルール】
        1. **文書全体の構造化**: タイトル、日付、注意書きなどの「表の外にある情報」も決して無視せず、適切な見出し（# や ##）や箇条書きを用いて必ず抽出に含めてください。
        2. **表データ (Tabular Data)**: 
           - 表がある場合は、Markdownのテーブル形式（|---|）を使用して抽出してください。
        3. **結合セルの完全補完**: 縦・横に結合されたセルがある場合、その内容を**対応するすべての論理的なセルに繰り返し入力**してください。
        4. **視覚的整合性の維持**: 各項目がどの列に属するかを、画像上の垂直・水平方向の位置関係から厳密に判断してください。
        5. **構造の維持**: 各行の「|」の数をヘッダーと一致させてください。空のセルには「 」を入れてください。
        6. **改行**: セル内で改行が必要な場合は `<br>` を使用してください。
        7. **言語**: 日本語を英語に翻訳しないでください。元の言語のまま抽出してください。
        8. **出力形式**: Markdownデータのみを出力し、説明は一切省いてください。
        """
