"""
リクルートダイレクトスカウト スカウトページの構造確認
実行: python check_directscout.py
"""
import asyncio
from playwright.async_api import async_playwright

LIST_URL = "https://directscout.recruit.co.jp/recommend?check_onboarding_status=1"
...
async def main():
    async with async_playwright() as p:
        ...
        links = await page.eval_on_selector_all(
            ...
            ".filter(l => l.href.includes('recruit') && !l.href.endsWith('#') && l.href !== window.location.href)"
        )
        seen = set()
        for l in links:
            h = l['href']
