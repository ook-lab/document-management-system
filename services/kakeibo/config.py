...
4. **税率・外税/内税の判定**
   - 商品行頭に「外8」「外10」がある → 外税（税抜価格表示）
   - レシートに「外税対象額」「外税額」の記載がある → 外税
   - 商品行頭に「内8」「内10」がある → 内税（税込価格表示）
   - 合計欄の「内消費税等」は外税計算後の税内包表示であり、内税の証拠ではない
   - 判定が難しい場合は商品価格の表示形式（税抜か税込か）を優先して判断
...
- **レシートに数値が印字されていれば、必ずそのまま出力すること。値の整合性チェックや妥当性判断は不要。読めた値をすべて記録する。**
- 推測や計算は一切行わないでください
