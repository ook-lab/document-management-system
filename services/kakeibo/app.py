...
# ═══════════════════════════════════════════════════
# 集計・分析ビュー（旧 kakeibo_view を統合）
# ═══════════════════════════════════════════════════

from collections import defaultdict

def _view_month_range(year_month):
    year, month = map(int, year_month.split('-'))
...
def _view_prev_next_month(year_month):
    year, month = map(int, year_month.split('-'))
...
def _view_months_between(from_ym, to_ym):
...
def _view_get_manual_map(db, ids):
    manual_map = {}
    for i in range(0, len(ids), 100):
        chunk = ids[i:i + 100]
...
def _view_check_auto_target(content, institution, auto_rules):
...
def _view_get_list_transactions(db, start_date, end_date, auto_rules, cat_rules, card_loan_rules):
    ...
    transactions = res.data
    ids = [t['id'] for t in transactions]
    manual_map = _view_get_manual_map(db, ids) if ids else {}
    ...
    for t in transactions:
        ...
        is_excluded = m.get('is_excluded', False)
        if not m:
            auto_action = _view_check_auto_target(content, institution, auto_rules)
            if auto_action == 'CASH_ONLY':
                is_excluded = True
        else:
            if view_target == 'CASH_ONLY':
        ...
        matched_card_rule = match_card_loan_rule(content, institution, amount, card_loan_rules)
        ...
        if not cat_major:
            suggested = _view_match_category_rule(content, cat_rules)
            if suggested:
...
def _view_get_cash_transactions(db, start_date, end_date, auto_rules, card_loan_rules, cash_cat_rules):
    res = db.table("Rawdata_BANK_transactions") \
        .select("*") \
        .gte("date", start_date) \
        .lt("date", end_date) \
    ...
    for t in transactions:
        ...
        cash_cat_mid   = m.get('cash_cat_mid') or ''
        if not cash_cat_major:
            cr = match_cash_category_rule(content, institution, amount, cash_cat_rules)
            if cr:
...
def view_index():
    ...
    return redirect(url_for('view_expense_summary'))
...
def view_expense_summary():
    ...
    year_month = request.args.get('month', datetime.now().strftime('%Y-%m'))
    db = get_db()
    auto_rules, cat_rules, card_loan_rules, cash_cat_rules = _view_get_rules(db)
    excluded_majors = _view_get_excluded_majors(db)

    start, end = _view_month_range(year_month)
    txs = _view_get_list_transactions(db, start, end, auto_rules, cat_rules, card_loan_rules)
    ...
    for maj, maj_amt in cat_sorted:
        ...
        for mid, sml_dict in sorted(
            hierarchy[maj].items(),
            key=lambda x: sum(sum(tx['amount'] for tx in txs_inner) for txs_inner in x[1].values())
        ):
            sml_list = []
            direct_txs = []
            for sml, txs_inner in sml_dict.items():
                if sml:
                    sml_amt = sum(tx['amount'] for tx in txs_inner)
                    sml_list.append((sml, sml_amt, sorted(txs_inner, key=lambda x: x['date'])))
                else:
                    direct_txs = sorted(txs_inner, key=lambda x: x['date'])
            sml_list = sorted(sml_list, key=lambda x: x[1])
            mid_total = sum(sum(tx['amount'] for tx in txs_inner) for txs_inner in sml_dict.values())
    ...
    prev_m, next_m = _view_prev_next_month(year_month)
...
def view_cash_summary():
    ...
    start, end = _view_month_range(year_month)
    txs = _view_get_cash_transactions(db, start, end, auto_rules, card_loan_rules, cash_cat_rules)
    txs = [t for t in txs if t['category'] not in excluded_majors]
...
@app.route('/view/multi_month')
def view_multi_month():
    """複数月サマリー（支出ベース）"""
    today = datetime.now()
    ...
    month_list = _view_months_between(from_ym, to_ym)
    ty, tm = map(int, to_ym.split('-'))
    start = f"{from_ym}-01"
...
def view_trend():
    ...
    cat_level  = request.args.get('cat_level', 'major')
...
@app.route('/view/api/trend_set/add', methods=['POST'])
def view_add_trend_set():
    data = request.json
...
@app.route('/view/api/trend_set/delete', methods=['POST'])
def view_delete_trend_set():
...
def view_settings():
    ...
    excluded = _view_get_excluded_majors(db)
    return render_template('settings.html', all_cats=all_cats, excluded=excluded)


@app.route('/view/api/settings/excluded_majors', methods=['POST'])
def view_update_excluded_majors():
    """除外大分類リストを一括更新"""
