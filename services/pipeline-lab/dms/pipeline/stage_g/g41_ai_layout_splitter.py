...
_G41_NON_RETRIABLE_MARKERS = (
    "g41_col_split_forbidden:",
    "g41_col_split_unnecessary:",
    "GOOGLE_AI_API_KEY_missing",
    "g41_col_split_header_row_invalid",
)

_G41_RETRIABLE_MARKERS = (
    "g41_col_blocks_overlap:",
    "g41_col_split_boundary_mismatch:",
    "g41_ai_normalize_failed",
    "g41_ai_json_parse_failed",
)


class G41LayoutAIRequiredError(RuntimeError):
    """F55: レイアウト理解（LLM）が必須だが契約を満たせない。"""


def _g41_layout_error_retriable(message: str) -> bool:
    """再プロンプトで直る見込みがあるエラーのみ True（構造違反は False）。"""
    if any(m in message for m in _G41_NON_RETRIABLE_MARKERS):
        return False
    if message == "g41_ai_normalize_failed" or any(
        m in message for m in _G41_RETRIABLE_MARKERS
    ):
        return True
    return False
...
def _ai_layout_env_enabled() -> bool:
    v = (
        os.environ.get("DMS_G41_AI_LAYOUT")
        or os.environ.get("DMS_F55_AI_LAYOUT", "")
    ).strip().lower()
...
def _validate_col_split_narrow_table(ncols: int, blocks: List[Dict[str, int]]) -> None:
    ...
    if ncols <= 3 and len(blocks) >= 2:
        raise G41LayoutAIRequiredError(
            f"g41_col_split_unnecessary: ncols={ncols} keep single table"
        )
...
def _validate_col_split_blocks(
    ...
    header = table[0]
    if not isinstance(header, (list, tuple)):
        raise G41LayoutAIRequiredError("g41_col_split_header_row_invalid")
    ...
    if b0["end"] != section1_end:
        raise G41LayoutAIRequiredError(
            f"g41_col_split_boundary_mismatch: block0 end={b0['end']} "
            f"expected {section1_end} (header title cols={non_empty})"
        )
    if b1["start"] != section1_end + 1:
        raise G41LayoutAIRequiredError(
            f"g41_col_split_boundary_mismatch: block1 start={b1['start']} "
...
def _normalize_detection(raw: Dict[str, Any], *, nrows: int, ncols: int) -> Optional[Dict[str, Any]]:
    ...
    if ncols <= 3 and cs:
        raise G41LayoutAIRequiredError(
            f"g41_col_split_forbidden: ncols={ncols} col_split must be false"
        )
    ...
    def _semantics_required(n_blocks: int) -> Optional[Dict[str, Any]]:
        ...
        if not isinstance(intent, str) or not intent.strip():
            logger.info("[G41 AI] whole_table_intent 欠落 → 棄却")
            return None
        summaries = raw.get("block_summaries")
        if (
            isinstance(summaries, list)
            and len(summaries) == n_blocks
            and all(isinstance(s, str) and s.strip() for s in summaries)
        ):
            cleaned = [str(s).strip() for s in summaries]
        else:
            # block_summaries は表示用メタデータ。長さ不一致・省略は空で続行（split は阻害しない）
            if summaries is not None:
                logger.info(
                    f"[G41 AI] block_summaries 長さ不一致（期待={n_blocks} 実際={len(summaries) if isinstance(summaries, list) else type(summaries).__name__}）→ 空で続行"
                )
            cleaned = [""] * n_blocks
        return {
            "ai_whole_table_intent": intent.strip(),
            "ai_block_summaries": cleaned,
        }
    ...
    for i in range(len(norm) - 1):
        if norm[i]["end"] >= norm[i + 1]["start"]:
            raise G41LayoutAIRequiredError(
                f"g41_col_blocks_overlap: {norm[i]!r} and {norm[i + 1]!r} "
...
def suggest_ai_table_split(
    ...
    *,
    ...
    if not api_key:
        if require:
            raise G41LayoutAIRequiredError("GOOGLE_AI_API_KEY_missing")
        logger.debug("[G41 AI] skip: GOOGLE_AI_API_KEY unset")
        return None
...
"""
    ...
    try:
        ...
        model = genai.GenerativeModel(G41_AI_LAYOUT_MODEL)
        for attempt in range(G41_AI_MAX_ATTEMPTS):
            prompt = prompt_base
            if last_err:
                prompt += f"\n## 修正必須（前回の契約違反）\n{last_err}\n"
            response = model.generate_content(prompt, request_options={"timeout": 120})
            raw_text = getattr(response, "text", None) or ""
            logger.info(f"[G41 AI] GENERATION attempt={attempt + 1}\n{raw_text[:8000]}")
            try:
                parsed = json.loads(_extract_json(raw_text))
            except json.JSONDecodeError as exc:
                last_err = f"g41_ai_json_parse_failed: {exc}"
                if not _g41_layout_error_retriable(last_err) or attempt >= G41_AI_MAX_ATTEMPTS - 1:
                    if require:
                        raise G41LayoutAIRequiredError(last_err) from exc
                    return None
                continue
            try:
                ...
                last_err = str(exc)
                if not _g41_layout_error_retriable(last_err):
                    if require:
                        raise
                    return None
                if attempt >= G41_AI_MAX_ATTEMPTS - 1:
                    if require:
                        raise
                    return None
                continue
            if candidate is None:
                last_err = "g41_ai_normalize_failed"
                if attempt >= G41_AI_MAX_ATTEMPTS - 1:
                    if require:
                        raise G41LayoutAIRequiredError(last_err)
                    return None
                continue
        ...
        if out is None:
            if require:
                raise G41LayoutAIRequiredError(last_err or "g41_ai_normalize_failed")
            return None
        ...
        try:
            ...
            logger.warning(f"[G41 AI] cost log failed: {_e}")
        return out
    except G41LayoutAIRequiredError:
        raise
    except Exception as e:
        if require:
            raise G41LayoutAIRequiredError(f"g41_ai_call_failed: {e}") from e
        logger.warning(f"[G41 AI] failed: {e}")
        return None
