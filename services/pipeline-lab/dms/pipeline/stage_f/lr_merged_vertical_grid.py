...
F51_GEOMETRY_CONTRACT = "f51_lr_vertical_geometry_v1"
F51_AI_JUDGE_CONTRACT = "f51_merged_cell_correspondence_v3"
...
def is_lr_merged_vertical_candidate(
    ...
    *,
    ...
    if len(plumber_rows) < header_rows + 2:
        return False
    if len(_column_x_bounds(plumber_rows)) < 5:
        return False
    if len(_collect_right_bands(plumber_rows, header_rows)) < 2:
        return False
...
def _left_y_clusters(
    ...
    *,
    ...
    words_item = _words_in_y_band(page, table_bbox, y_body0, y_body1, x_bounds[0][0], x_bounds[0][1])
    words_amt = _words_in_y_band(page, table_bbox, y_body0, y_body1, x_bounds[1][0], x_bounds[1][1])
    all_w = sorted(words_item + words_amt, key=lambda w: float(w["top"]))
    if not all_w:
        return []
    ...
    for w in all_w:
        ...
        if not clusters:
            clusters.append([w])
            continue
        prev_cy = sum((float(x["top"]) + float(x["bottom"])) / 2.0 for x in clusters[-1]) / len(clusters[-1])
    ...
    for cl in clusters:
        ...
        item = _join_words([w for w in cl if float(w["x0"]) < x_bounds[1][0]])
        amt = _join_words([w for w in cl if float(w["x0"]) >= x_bounds[1][0] - 1])
...
def _build_row_aligned_body(
    ...
    for ri in range(header_rows, len(plumber_rows)):
        ...
        y0, y1 = y_band
        left_item = _join_words(_words_in_y_band(page, table_bbox, y0, y1, x_bounds[0][0], x_bounds[0][1]))
        left_amt = _join_words(_words_in_y_band(page, table_bbox, y0, y1, x_bounds[1][0], x_bounds[1][1]))
...
def _build_block_common_body(
    ...
    left_merge = _left_merged_bbox(plumber_rows, header_rows)
    if not left_merge:
        raise LRMergedVerticalRebuildError("block_common_no_left_bbox")

    y0, y1 = left_merge
    common_item = _join_words(_words_in_y_band(page, table_bbox, y0, y1, x_bounds[0][0], x_bounds[0][1]))
    common_amt = _join_words(_words_in_y_band(page, table_bbox, y0, y1, x_bounds[1][0], x_bounds[1][1]))
    if not common_item and not common_amt:
        raise LRMergedVerticalRebuildError("block_common_no_left_text")
    ...
    for ri in range(header_rows, len(plumber_rows)):
        ...
        exp_total = _cell_text(row_extract, 3)
        per_cap = _cell_text(row_extract, 4)
        if not exp_item and not exp_total and not per_cap:
            continue
        if first:
            body.append([common_item, common_amt, exp_item, exp_total, per_cap])
            first = False
        else:
            body.append(["", "", exp_item, exp_total, per_cap])
    if not body:
        raise LRMergedVerticalRebuildError("block_common_body_empty")
    return body
...
def rebuild_lr_merged_vertical_table(
    ...
    *,
    ...
    """F51 geometry 判定済み mode で格子を機械再構成する。"""
    ...
    if judge not in (F51_GEOMETRY_CONTRACT, F51_AI_JUDGE_CONTRACT):
        raise LRMergedVerticalRebuildError("f51_judge_contract_missing")
    logger.info(f"[F51] lr_merged_vertical: mode={mode} {len(data)}→{len(out)}行")
    return out, meta
...
def grid_has_unprocessed_left_stack(
    ...
    *,
    ...
    """F51 未適用の左列改行スタック（5列以上・geometry 痕跡）。語は見ない。"""
    if _max_cols(grid) < 5:
        return False
