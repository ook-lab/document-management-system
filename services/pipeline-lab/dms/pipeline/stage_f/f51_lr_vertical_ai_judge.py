...
契約: f51_merged_cell_correspondence_v3
"""
...
from dms.pipeline.stage_f.lr_merged_vertical_grid import VerticalMergeMode
from dms.pipeline.stage_f.merged_cell_grid import RowMergeMeta
...
class F51LRVerticalAIError(RuntimeError):
    """F51 結合セル AI の契約違反・呼び出し失敗。"""
...
def _cells_from_logical_item(item: Dict[str, Any], index: int) -> List[str]:
    if "cells" in item:
        ...
        if not isinstance(raw, list) or not raw:
            raise F51LRVerticalAIError(f"f51_ai_logical_row_empty_cells: index={index}")
        return [_cell_str(c) for c in raw]
    if all(k in item for k in _ROW_KEYS):
        return [_cell_str(item[k]) for k in _ROW_KEYS]
    raise F51LRVerticalAIError(f"f51_ai_logical_row_missing_cells: index={index}")
...
def _parse_horizontal_merges(
    ...
    *,
    ...
    if not isinstance(raw, list):
        raise F51LRVerticalAIError("f51_ai_horizontal_merges_invalid")
    out: List[Dict[str, Any]] = []
    for item in raw:
        if not isinstance(item, dict):
            raise F51LRVerticalAIError("f51_ai_horizontal_merge_item_invalid")
        try:
            ri = int(item["row_index"])
            start = int(item["start_col"])
            span = int(item["colspan"])
        except (KeyError, TypeError, ValueError):
            raise F51LRVerticalAIError("f51_ai_horizontal_merge_fields_missing")
        if span < 2:
            raise F51LRVerticalAIError(f"f51_ai_horizontal_merge_span_too_small: row={ri}")
        if ri < 0 or ri >= n_rows or start < 0 or start + span > ncols:
            raise F51LRVerticalAIError(f"f51_ai_horizontal_merge_out_of_range: row={ri}")
...
def _parse_vertical_merges(raw: Any) -> List[Dict[str, Any]]:
    if raw is None:
        return []
    if not isinstance(raw, list):
        raise F51LRVerticalAIError("f51_ai_vertical_merges_invalid")
    out: List[Dict[str, Any]] = []
    for item in raw:
        if not isinstance(item, dict):
            raise F51LRVerticalAIError("f51_ai_vertical_merge_item_invalid")
        note = item.get("description") or item.get("note")
        if not isinstance(note, str) or not note.strip():
            raise F51LRVerticalAIError("f51_ai_vertical_merge_missing_description")
        out.append(
            {
...
def _parse_ai_correspondence(parsed: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if kind not in _VALID_LAYOUT:
        raise F51LRVerticalAIError(f"f51_ai_invalid_layout_kind: {kind!r}")

    summary = parsed.get("correspondence_summary")
    if not isinstance(summary, str) or not summary.strip():
        raise F51LRVerticalAIError("f51_ai_missing_correspondence_summary")

    rationale = parsed.get("rationale")
    if not isinstance(rationale, str) or not rationale.strip():
        raise F51LRVerticalAIError("f51_ai_missing_rationale")

    try:
        conf = float(parsed.get("confidence", 0))
    except (TypeError, ValueError):
        raise F51LRVerticalAIError("f51_ai_missing_confidence")
    if not (0.0 <= conf <= 1.0):
        raise F51LRVerticalAIError("f51_ai_confidence_out_of_range")
    ...
    if kind in _NO_MERGE_KINDS:
        ...
        if lr is not None and lr != []:
            raise F51LRVerticalAIError(
                "f51_ai_contradiction: no_merge_with_logical_rows"
            )
        return out

    logical_raw = parsed.get("logical_rows")
    if not isinstance(logical_raw, list) or not logical_raw:
        raise F51LRVerticalAIError("f51_ai_missing_logical_rows")

    logical: List[List[str]] = []
    for i, item in enumerate(logical_raw):
        if not isinstance(item, dict):
            raise F51LRVerticalAIError(f"f51_ai_logical_row_invalid: index={i}")
    ...
    if kind == "full_resolution" and logical:
        ...
        if non_empty == 0:
            raise F51LRVerticalAIError("f51_ai_empty_logical_rows")
    ...
    if "extract_header_row" in parsed:
        ...
        if raw_ehr is None:
            ...
            try:
                ...
                raise F51LRVerticalAIError("f51_ai_invalid_extract_header_row") from e
    else:
    ...
    if "header_cells" in parsed:
        ...
        if raw_hc is None:
            ...
            raise F51LRVerticalAIError("f51_ai_header_cells_invalid")

    if kind == "block_common":
        lb = parsed.get("left_block")
        if not isinstance(lb, dict):
            raise F51LRVerticalAIError("f51_ai_missing_left_block")
        left_block: Dict[str, str] = {}
        for k in _LEFT_BLOCK_KEYS:
            if k not in lb:
                raise F51LRVerticalAIError(f"f51_ai_left_block_missing_{k}")
    ...
    for row in logical:
        for cell in row:
            if " / " in _cell_str(cell):
                raise F51LRVerticalAIError(
                    "f51_ai_slash_joined_cell: use separate logical_rows per item"
                )
    for vm in ai.get("vertical_merges") or []:
        ...
        for row in logical:
            if ac < len(row) and " / " in _cell_str(row[ac]):
                raise F51LRVerticalAIError(
                    "f51_ai_slash_join_contradicts_vertical_merge"
                )
...
def judge_lr_vertical_layout_ai(
    *,
    ...
    if not api_key:
        raise F51LRVerticalAIError("GOOGLE_AI_API_KEY_missing")

    if not table_preview:
        raise F51LRVerticalAIError("f51_ai_empty_table_preview")
...
"""
    ...
    try:
        ...
        raise F51LRVerticalAIError(f"f51_ai_json_parse_failed: {e}") from e
    except F51LRVerticalAIError:
        raise
    except Exception as e:
        raise F51LRVerticalAIError(f"f51_ai_call_failed: {e}") from e
    ...
    try:
        ...
        log_ai_usage(
            ...
            model=F51_LR_VERTICAL_AI_MODEL,
        ...
        logger.warning(f"[F51-AI] cost log failed: {e}")

    logger.info(
        f"[F51-AI] kind={out['layout_kind']} logical_rows={len(out.get('logical_rows') or [])} "
...
def rebuild_grid_from_ai_correspondence(
    ...
    *,
    ...
    if ai.get("layout_ai_contract") != F51_LR_VERTICAL_AI_CONTRACT:
        raise F51LRVerticalAIError("f51_rebuild_contract_mismatch")

    kind = ai.get("layout_kind")
    if kind in _NO_MERGE_KINDS:
        raise F51LRVerticalAIError("f51_rebuild_called_for_no_merge")

    logical: List[List[str]] = ai.get("logical_rows") or []
    if not logical:
        raise F51LRVerticalAIError("f51_rebuild_empty_logical_rows")
    ...
    if ehr is None:
        ...
        if hc is None:
            raise F51LRVerticalAIError("f51_rebuild_missing_header_cells")
        ...
        if ai.get("header_cells") is not None:
            ...
            raise F51LRVerticalAIError(f"f51_rebuild_extract_header_row_oob: {ehr}")
    else:
        raise F51LRVerticalAIError("f51_rebuild_invalid_extract_header_row")
