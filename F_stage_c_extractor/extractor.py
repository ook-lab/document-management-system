...
# 【🚨 最重要原則 🚨】情報量の完全保持 - 削減は絶対禁止

**【情報量の検証】あなたの出力を検証してください**:
- 元の文書に1000文字のテキストがあれば、JSONのどこかに1000文字分が格納されているべきです
- 元の文書に表が30行あれば、JSONのどこかに30行分のデータが格納されているべきです
- **元の情報量 ≒ 抽出後の情報量** でなければなりません
- 情報が大幅に減っている場合、それは抽出の失敗です

**【2段階配置ルール】全テキストを必ずどこかに配置**:
文書内の全ての情報は、以下の2段階で**必ず**格納してください:

1. **第1段階: 専用フィールドを探す**
   - monthly_schedule_blocks, event_details, learning_content_blocks 等の専用フィールドに該当する情報はそこへ

2. **第2段階: text_blocks/structured_tablesに必ず格納（最後の砦）**
   - 専用フィールドに該当しない情報は**text_blocks または structured_tables に必ず格納**
   - text_blocks: あらゆる文章（長文、短文、箇条書き、説明文、全て）
   - structured_tables: あらゆる表（専用フィールドに該当しない全ての表）
   - **「どこにも該当しないから省略」は絶対に許されません**

**【🚨絶対禁止事項🚨】以下は重大な違反です**:
- ❌ **情報の削除・削減・省略** ← これは最悪の違反行為です
- ❌ 「専用フィールドに該当しないから省略」
- ❌ 要約・言い換え（metadataは「生データ」として原文そのまま）
- ❌ 「代表例だけ抽出して残りは省略」
- ❌ 配列の一部抽出（表の全30行を抽出、一部省略は禁止）

**【情報保持の具体例】**:
- コンサートの案内 → event_detailsに詳細を格納 + プログラムや注意事項の全文をtext_blocksにも格納
- 月間予定表 → monthly_schedule_blocksに全行を格納（1行も省略しない）
- 先生からのメッセージ → text_blocksに全文を格納（要約ではなく原文コピー）
- 持ち物リスト → structured_tablesに全項目を格納

**その他のルール**:
- 推測や補完は不要。記載されている情報のみを忠実に構造化
- 日付は必ずYYYY-MM-DD形式で統一
- 見つからない情報はnullまたは空のリスト[]を設定
...
🚨🚨🚨 【最終確認】出力前に必ずチェック 🚨🚨🚨:
□ 元の文書のテキスト量と、JSON内のテキスト量は同等か？（90%以上保持されているか？）
□ 専用フィールドに該当しない情報は、全てtext_blocks/structured_tablesに格納したか？
□ 表の行を省略せず、全行を抽出したか？
□ contentフィールドは原文コピー&ペーストで、要約・言い換えをしていないか？

**重要: 情報の削除・削減・省略は一切禁止です。元のテキスト量とJSON内のテキスト量が同等になるよう、原文の全量をJSON構造に落とし込んでください。**
...
それでは、上記の注意事項を守って、JSONを出力してください:"""
   ...
   **文章は text_blocks へ、時間割は weekly_schedule へ振り分けてください。**
   ...
   metadataフィールドの構造:
   {
     ...
     "event_details": {
       "event_name": "イベント名（例: 洗足クリスマスファミリーコンサート）",
       "date": "YYYY-MM-DD",
       "time": "開場/開演時刻（例: 開場13:30 開演14:00）",
       "venue": "会場名（例: 洗足学園前田ホール）",
       "performers": ["出演者1", "出演者2"],
       "program": [
         {"performer": "出演者名", "songs": ["曲1", "曲2"]}
       ],
       "ticket_info": "チケット情報全文（原文そのまま）",
       "application_method": "申込方法全文（URL含む、原文そのまま）",
       "notes": "注意事項全文（原文そのまま）"
     },
     "special_events": [
       "特別イベント1",
   ...
   1. **text_blocks**: 🚨 情報保持の最後の砦 🚨
      - **役割**: 専用フィールドに該当しない全てのテキストを格納する受け皿
      - **対象**: あらゆる文章（長文、短文、箇条書き、説明文、注意事項、メッセージ、全て）
      - 朝会の話、今日のふりかえり、道徳の内容、先生からのメッセージ、連絡事項、お知らせ
      - 学級通信や学年通信の記事、お知らせ本文、箇条書きの連絡事項
      - コンサート・イベントのプログラム詳細、チケット情報、申込方法、注意事項
      - **専用フィールドに入れた情報でも、詳細が長い場合はtext_blocksにも原文を重複格納してOK**
      - 見出しがない場合は適切なタイトルをつける（例: 「連絡事項」「お知らせ」「持ち物について」「プログラム詳細」）
      - title（見出し）と content（本文全文）のペアで記録

      **【最重要】content フィールドは要約・説明厳禁、原文コピー&ペーストのみ**:
      - ❌ NG例: "冬休み明けに実施されるテストについて予告されています" ← これは要約・説明文であり、使い物になりません
      - ❌ NG例: "道徳の授業について記載されています" ← これも要約であり、内容が何もありません
      - ❌ NG例: "代表委員の募集に関する内容です" ← これも説明文であり、具体的な情報がゼロです
      - ✅ OK例: "1月8日（水）に国語と算数のテストを実施します。範囲は教科書p.50-80です。ノート・定規を持参してください。" ← 元の文書の文章をそのままコピー
      - ✅ OK例: "今日の道徳の授業では「思いやり」について考えました。隣のクラスの友達が困っているときにどう声をかけるか、グループで話し合いました。田中さんが『一緒に探そうか』と言ったのが印象的でした。" ← 元の文書の文章を一字一句そのまま

      **あなたがやるべきこと**:
      - 元の文書からテキスト部分を見つける
      - そのテキストを**一字一句コピー&ペースト**する
      - 要約しない、言い換えない、省略しない

      **あなたがやってはいけないこと**:
      - 「〜について記載されています」のような説明文を書く
      - 内容を要約する
      - 自分の言葉で言い換える

      - 例: [
          {"title": "朝会「マナーとルールについて」", "content": "今週の朝会では、学校生活におけるマナーとルールについて話しました。廊下を走らないこと、友達に優しくすることの大切さを確認しました。特に低学年の子たちが困っているときは、高学年として手を差し伸べてあげましょう。"},
          {"title": "今日のふりかえり", "content": "今日は算数の時間に分数の計算を学びました。最初は難しかったですが、田中先生の図を使った説明でよく分かりました。宿題はプリント1枚です。"},
   ...
   【🚨絶対原則🚨】:
   - **情報の削除・削減・省略は一切禁止** ← 最重要
   - **元のテキスト量 ≒ JSON内のテキスト量** を維持すること
   - 原文の全量を構造化データに落とし込む
   ...
   - **【text_blocksを最後の砦として使用】**: 専用フィールドに該当しない文章は、全てtext_blocksに格納すること
   - **【重複格納OK】**: event_detailsなどの専用フィールドに入れた情報でも、詳細が長い場合はtext_blocksにも原文を格納してOK（情報の保持を最優先）
   - 日付は必ず YYYY-MM-DD 形式で統一
   - 見つからない情報は null または空のリスト [] を設定

   【重要】文章と時間割の振り分け:
   - **文章（記事、お知らせ本文、メッセージ、イベント詳細）→ text_blocks（最後の砦として必ず使用）**
   - **時間割・予定表（授業、行事スケジュール）→ weekly_schedule**
   - **その他の表（持ち物、価格表、献立など）→ structured_tables**

   **疑問があったら text_blocks へ**: どのフィールドに入れるか迷ったら、とにかく text_blocks に入れてください。情報を捨てるよりも、text_blocks に入れる方が100倍マシです。
        """
